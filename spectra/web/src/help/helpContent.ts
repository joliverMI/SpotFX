...
export const HELP_SECTIONS: HelpSection[] = [
  ...
  },
  {
    ...
    entries: [
      ...
      {
        id: 'flare-kinds',
        title: 'Named flare kinds — the four types',
        keywords: 'kind drift jump momentary permanent slam scale strength declare card offset relative random hold duration target reverse toggle boolean color rotate rotate-and-back hue foreground degrees dwell ramp fade star radial spin flip spin_sign sign instant no pause freeze',
        body: [
          'A scene DECLARES named flare kinds — readable cards at the top of the response tabs, shared by every class. Four types: DRIFT-JUMP (🎨/🎲) jumps the drift itself — the colour-set jump through the shipped selector, or a dice re-roll for shape; both carry, the journey walks on from the new point. MOMENTARY (↩) spikes params/gain and RETURNS exactly to the carried baseline. PERMANENT (⚓) lands and BECOMES the new baseline drift carries from. COLOR ROTATE-AND-BACK (🌈) spins the foreground colour\'s hue and returns it — see below.',
      ...
      },
      {
        ...
        body: [
          'OPENING THIS PREVIEW CHANGES YOUR REAL LIGHTS. It fires this card\'s own scene onto your fixtures and holds it there, then fires this one flare kind on top of it for real — not just in the scrub timeline below. Your trigger engine is paused for as long as the preview stays open, so if a song is playing, its scene changes, other flares, and charge/lull/drop moments all go silent until you close this window: you are temporarily replacing your live show with this one scene + flare, on purpose, to look at it. Closing the preview (or navigating away) reverts your room to exactly what it was showing the instant you opened it, and your live show resumes from wherever it would otherwise be. If you close the browser tab or lose connection instead of pressing close, the app notices on its own and reverts anyway — worst case about 17 seconds after your last moment on the page, never longer, whether or not you ever press close.',
          'Tap ▶ Preview on any flare kind card to open its scrubbing timeline: a ruler that plays across and loops, showing exactly what this one kind does, in isolation from any band. Play/Pause, then drag the playhead anywhere on the ruler to freeze-frame the effect mid-animation. Extend widens the ruler without changing anything already on it. The intensity slider at the top recomputes the timeline AND re-fires the scene + kind live at the new strength.',
          ...
          'The scrub ruler itself is still computed, not a live camera on the room: it\'s the same execution model the response engine always runs, driven once by hand instead of by the music, so scrubbing/looping/dragging the playhead never sends anything further to your fixtures — only opening (and re-fires on an intensity change) do that.',
      ...
      },
      {
        ...
        body: [
          ...
          'Fireworks: charge — launch rate climbs 6× while bursts shrink and slow; lull — launching stops, six dim rockets cross the dark panel, travelling about twice as far past centre as before exploding; drop — every rocket explodes where it is, giant, in its own colour.',
