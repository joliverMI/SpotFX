...
export const HELP_SECTIONS: HelpSection[] = [
  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        ...
        keywords: 'fish school ripple wake flap tail turn radius oval swim pond rush orbits copy disperse fade swirl lull burst swim burst flare dash stroke thrust pulse coast drift bend body',
        body: [
          'Fish is Orbits\' twin: the same scene, the same flare kinds and bands, the same initial values, weightings and curves — a copy, not a re-tuning. What differs is the creature. Each one is a thin oval seen from above whose head POINTS the way it is swimming; the rest of its body follows the path it actually swam, so it bends through a turn instead of pivoting like a stick. Its spine flaps as it goes, and it leaves an expanding ripple wake instead of Orbits\' comet smear. It turns through a real circle: nothing can spin a fish faster than its Turn Radius allows, so an about-face is always an arc and never a flip on the spot.',
          'Avoid Each Other (added 2026-08-28) is how hard they steer clear of a neighbour that gets close. It is STEERING ONLY: it just adds one more pull to where a fish wants to point, and that still passes through the same turn-rate limit as everything else — so avoidance can never reverse a fish on the spot, never tighten its circle past its Turn Radius, and never nudge one sideways. The distance they start dodging at is not a knob: it comes from their own body length, so bigger fish take more room automatically. At 0 they swim straight through one another, exactly as they did before this existed. It is deliberately OFF during the charge\'s school (they are meant to move almost identically) and during the drop\'s rush (which is meant to be chaotic) — those are authored moments, not crowds to fix.',
          ...
          'The tail stroke is what drives a fish forward: its speed surges on each stroke and coasts between them, and slower music means slower strokes, so the pushes read further apart. Min Drift Speed is the speed it keeps between strokes, as a share of its ordinary swim speed; Stroke Speed Cap is how much extra one stroke can add. The way back is built in: Min Drift Speed at 1 with Stroke Speed Cap at 0 is the old smooth, un-pulsed swimming exactly. The charge\'s school and the drop\'s rush never pulse — those are authored moments.',
          'The wake is a trail like Orbits\', laid continuously at the tail, that spreads outward as it fades — no rings and no outline. Ripple Wake sets its strength — always subtle against the fish itself, stronger the faster it swims, and textured by each tail beat. Ripple Spread is how fast the trail opens outward as it fades, Ripple Life how long it takes to fade, Ripple Width how wide each deposit is. Wake Length is separate — that is the fish\'s own smear, not the spreading trail.',
          'The remaining knobs shape the charge and the drop (see "Response families"): School Size, School Variation and School Turn Gap for the charge; Rush Size, Rush Time and Rush Chaos for the drop.',
          'Fish never fade out. Whenever fish have to go — the population shrinking, the lull, or the drop\'s extra rush fish — they DISPERSE: each keeps its full brightness and swims out of the view, fast enough to be gone in time (the old fade\'s 1.2 seconds for an ordinary departure), and only once it has fully left is it removed.',
          'A scene change away from Fish depends on what comes in. An effect with blobs of its own — Black Hole, Orbits, Fireworks, Squiggles, Eye, Dancers, or Fish again — takes the fish as its own blobs, exactly as before. Radial gathers them into its bloom. Anything else (Pacman included, for now) has nothing to merge them into, so the fish swim off the view within the first ~60% of the crossfade. One honest limit on your crystal\'s Add blend: the blend dims whatever is leaving, and the brightest core of a fish is already at full, so the fish do dim somewhat as they go — to about 0.8 of their brightness a fifth of the way in — even though they are swimming off rather than fading on the spot.',
          'Swim Burst (the Fish Swim Burst flare) makes every fish dash — about three times its speed — and beat its tail several times faster, for the flare\'s own length. It is an ordinary momentary flare that fires on the trigger: its hold (300ms) is how long the burst lasts, and its trigger offset (0ms) is where it starts relative to the trigger. Ask Sonic to change either — e.g. "make the fish burst 400ms" — and the other number stays as it was. It sits in each flare band\'s Shape lane, so each flare picks the burst OR Reverse, never both. The burst stays on the screen: a dashing fish heading out toward the edge of its pond eases back to its ordinary speed so it can turn in time, while a fish clear of the edge, or already swimming back in, keeps the full dash.',
      ...
      },
      {
        ...
        body: [
          ...
          'THE SAVED OFFSET RETIMES YOUR REAL SHOW, not just this preview. Once the scene is Saved, any of your own song triggers that fires this scene\'s flare response actually fires early or late by this offset — the engine reads the offset of whichever kinds the fired band carries, moves the fire by it, and only then applies the same automatic head-start the "start" line shows here, so a crossing in the real show lands exactly where this ruler says it will. If a band carries several kinds with different offsets, each kind lands at its OWN moment: the fire begins at the EARLIEST offset in the band — a kind still at 0 counts like any other — and every other kind waits the difference before it fires. So a kind at -100 beside kinds at 0 fires 100 ms early by itself and the rest land on the mark, and a kind at +50 beside a kind at 0 lands 50 ms after the mark while the 0 kind fires on it — each at the spot its own trigger line sits here. Bridge-relayed legacy flares, the update flare a held scene change fires, and a manual test-fire are the exceptions: they arrive at their own moment with no advance notice, so nothing is moved and their whole band fires together at that moment — same reason automatic song-transition fires take no head start.',
      ...
      },
      {
        ...
        body: [
          ...
          'THE MARKS HERE ARE NOT DRAGGABLE, and that is deliberate. Each mark is its band\'s ANCHOR — the earliest offset among the flare kinds attached to that band, a kind still at 0 included, so a charge, lull or drop only starts later than its mark once EVERY kind in its band is set later — while every kind in the band keeps its own offset, so a drag would have to pick one kind to write it to, and picking for you would be inventing an answer. Retime a kind from its own ▶ Preview on the Flares or Charges/Lulls/Drops tab and this ruler shows the result: each class\'s mark moves to its band\'s anchor, which is where that fire begins, and the kinds inside it land at their own moments relative to that mark — a kind authored later than the anchor fires that much after it, here and in your show alike.',
      ...
      },
      {
        ...
        body: [
          'Each band shows a row of vertical LANES — 2 to start, a "+" grows up to 4. When the band fires, EVERY lane fires together — and a lane holding SEVERAL kinds picks exactly ONE of them, at random with even odds, fresh on every fire (the SpotFX morph-lane behaviour: each lane is a pool of alternatives, one pick per lane, all picks fire on the same fire — together, unless one of your song triggers fires the band and the picked kinds carry different trigger offsets, in which case each lands at its own; see the lane ORDER note below). A lane holding a single kind just fires that kind every time — so until you deliberately put two kinds into one lane, a band behaves exactly as it always has: everything attached fires.',
          ...
          'Lane ORDER still matters, exactly as before, among kinds that land at the SAME moment: it decides what happens when they touch the SAME parameter. A BATCH is every kind that lands at one moment — the whole band, unless one of your song triggers fires it and its kinds carry different trigger offsets, in which case each offset lands as its own batch. The engine\'s existing rule, per batch: a dice re-roll, a colour jump, a colour rotate, a firework burst and a blob rush are each a SINGLE pick per batch (two in one batch is harmless but only the first executes); momentary/permanent PARAM moves overwrite — permanent kinds land first, momentary lands after (so a spike shows over the just-set baseline), and among same-type kinds the LATER lane wins a shared param; a permanent GAIN kind chained after another permanent gain in the same batch multiplies onto it rather than replacing it. Kinds in DIFFERENT batches simply land in time order, so between them the LATER-landing kind has the last word, whatever lane it sits in. So within a batch a later lane is a deliberate "wins" for a param conflict, not an accident of drag order — and pooling never reorders anything: it only decides WHICH of a lane\'s members fires.',
      ...
      },
      {
        ...
        body: [
          ...
          'Fish: charge — up to eighteen fish (School Size) swim in and lock into one heading, spread evenly across the panel, and from then on the whole school changes direction on every beat, never closer together than 400ms, each fish varying just slightly from the shared heading. Lull — the ordered school breaks into a chaotic swirl round the centre of view, and as it swirls the fish leak out one by one and swim OFF the screen: every one is gone by the first third of the lull (so well before half way), the ripples they left linger to two thirds, and the last third is dark. They never fade out — a fish is at full brightness until its whole body has left the panel. A very short lull has no time to swirl, so the fish scatter straight out instead. Drop — up to twenty rush in from every side, swirl round the centre for the drop, and the scene\'s own Fish count stays behind; the payoff ejecta burst out of the centre as before. The school and the rush are the only two moments a fish scene exceeds its own population limit.',
    ...
    entries: [
      ...
      },
      {
        ...
        body: [
          ...
          'It tells you three things. WHO: if a capture run is executing right now — a light-field map, a commissioning pass, an exposure comparison, a pose fingerprint — the bar names THAT run and what it is on, because that is the one thing it can actually measure; otherwise a declared take names the agent and what it said it was testing, and failing both, whichever of SPECTRA\'s own preview paths is holding your room. SINCE WHEN: your own wall clock, plus how long it has been running once that is over a minute. AND WHETHER IT IS ACTUALLY PAINTING: "driving your lights (frames flowing)" means your room really is being rendered; "holding the room but NOT painting — that\'s a fault, not a test" means somebody has taken your lights and they have gone dark, which is a problem, not testing. Those two are different facts, which is why the bar states which one it is — an owner indicator alone once read green right through an outage.',
          'A live run outranks a declared take, and that is a fix rather than a preference (2026-09-05): a declaration is a note somebody typed, good for up to an hour, and it stays up after the run it was written for has finished. So a second run starting inside an earlier run\'s declaration used to wear the EARLIER run\'s reason while a completely different run held the room. A run now says what it is for itself, for exactly as long as it is running, and the "since" clock beside it belongs to whatever the line names. The declared note is not thrown away — it is still on the record — it just no longer speaks for a run it was not about.',
      ...
      {
        id: 'dark-fixture-watch',
        title: 'A light that is dark or not answering while SPECTRA streams to it',
        keywords: 'dark fixture watch dark light off switched off on=false brightness zero unreachable dropped off network gone not answering no reply slow busy timeout not receiving live=false streaming streamed tv backlight tv-backlight wled strip mid-show mid song silent fault named blind spot healthy liveness devices online false healthy sweep 30 seconds 60 seconds recovered came back read back json state json info',
        body: [
          'WHAT YOU SEE: an amber strip at the top of every page — "⚠ 1 light is dark or not answering while SPECTRA streams to it" — naming the light, saying which of four things is wrong, and how long it has been that way. Two more times sit beside that: "last confirmed dark" is the most recent read that actually said dark or did not answer (a lost reply does not move it), and "watch swept" is when the watch last asked at all. Read them together — "watch swept" growing past a minute means the watch itself has stalled; "last confirmed dark" growing on its own while "watch swept" stays fresh means the light is dropping replies but is still being watched. It appears whether or not there was ever a take-back problem, and it is a different strip from "A light the take-back had to skip" above only because the two catch the fault at different moments: that one can only speak about lights that were already dark when SPECTRA took the room, this one about a light that was fine and then went.',
          'WHAT IT MEANS: SPECTRA is pushing frames at that fixture right now, and the fixture itself either says it is showing nothing or will not say anything at all. Four sentences, four different next steps, and each claims only what the read actually established: "is not answering" — it did not reply to SPECTRA\'s read within 3 seconds while being streamed to. That is all that sentence knows: the light may be off the network (power, WiFi), or its controller may simply be too busy taking the stream to answer, and whether it is actually lit cannot be read until it answers again — that one, look at the wall; "answers but reports it is NOT receiving SPECTRA\'s stream" — it is up and talking but not taking the data (realtime disabled on it, or a blocked port); "is switched OFF at the fixture" — it is taking the stream and is powered off in its own firmware, so the data lands on a dark strip (something turned it off outside SPECTRA — the WLED app, a Home Assistant automation, its own timer); "its master brightness is 0" — the same, one knob over.',
          'WHY IT EXISTS: every other health reading in this app can say FINE over exactly this state, and did, twice, on the same fixture. Frames flowing proves SPECTRA\'s render loop pushed a frame, not that anything received it. The device "online" flag on the liveness endpoint is set once when the device is built and, for a WLED, essentially never clears — a fixture off the network for hours still reads online. And the take-back report only ever re-asks lights it already named. So a light going dark mid-show was invisible until someone looked at the wall. This watch reads the fixture\'s own answer back every 30 s and names it once it has read that way for a full minute — so a genuine fault shows up within about a minute and a half, and a single missed reply never accuses a working light. Nor does a missed reply ever clear a named one: only a read that positively says lit does that, and until then the strip and the Status line say "reading dark or not answering, not yet named" rather than counting the light as confirmed lit. A light whose reply was lost on one pass shows as "unchecked this sweep" and stays watched; "not checkable" is only ever said of a device the watch cannot ask at all (one that is not a WLED, or whose driver never got a client).',
          'NOTHING IS BEING WRITTEN TO FIX IT, deliberately. This is a report, not a repair: it never powers a fixture on, never changes its settings, never restarts anything, and it is never counted as "unhealthy" on the liveness endpoint — a restart cannot switch a light back on and would darken every light that IS working. The strip clears itself the moment the fixture reads lit again. It also stands down entirely, with nothing shown, while a Preview or a capture/mapping run holds the room (those move fixture power and brightness on purpose) and while SPECTRA does not own the lights.',
        ],
      },
      {
        id: 'param-watchdog',
      ...
      },
      {
        ...
        keywords: 'release home assistant ha panic emergency let go stop wled hue ddp virtuals band scope scoped bathroom other room house wide never streamed untouched left alone entertainment group',
        body: [
          ...
          'It only ever lets go of lights SPECTRA was actually driving. The Hue fade reaches every entertainment group this session streamed to and no others, so a group that was never brought up — a run scoped to one room, a group SPECTRA has not touched since it started — is not faded, not switched off, and not written to at all. A bulb in another room of the house therefore stays exactly as you left it. The unit is the entertainment GROUP rather than the individual bulb, and that is a property of Hue rather than a choice here: one entertainment stream carries every channel of its group in one packet, so streaming any part of a group drives all of it, and every bulb in a group SPECTRA streamed is owed the same letting-go.',
          ...
          'Driving the take-back from outside the app (a script, an agent running a capture): POST /api/ownership/handover accepts an OPTIONAL room_id and/or carrier_ids, and a take that names them brings up ONLY the fixtures behind those carriers — everything else in your config is loaded but never driven, so it is never streamed to and never switched off when the room is handed back. The button here sends neither, so pressing it is the whole room exactly as it always was. A named scope that cannot be placed against your fixture config is REFUSED (422) rather than quietly widened: asking for a narrow take and silently getting the whole house is the thing this exists to prevent.',
        ],
      },
      {
      ...
      },
      {
        ...
        body: [
          ...
          'This toggle now controls only the FALLBACK stage of the placement rule described in "Transition placement rule" below: it decides whether a cue that finds no rhythmic bass-energy edge nearby snaps to the nearest downbeat instead, or is left at its raw section-boundary time. The rhythmic-edge search itself always runs — see that topic for the full rule.',
          'Off: a generated cue that finds no edge keeps exactly its raw section-boundary time, unsnapped, rather than falling back to the nearest downbeat.',
          'Which downbeat grid: librosa\'s own beat grid (already computed for every analyzed song) is the default. If the song has been precomputed with beat_this (the test bed\'s second engine) AND beat_this reads its tempo at roughly half of librosa\'s — the phrase-length grid some genres are actually marked to — that phrase grid is used instead. Nothing here ever runs beat_this live; a song without a precompute just uses librosa.',
      ...
      },
      {
        ...
        keywords: 'transition placement rule edge window sensitivity density strongest per song beat bass energy generator generated cue mid-song align alignment R3',
        body: [
          'Three settings in the Scenes panel (room controls bar, above "Snap generated cues to beat"): "Transition edge window", "Transition edge sensitivity", and "Transitions per song". Together with the snap toggle, they decide WHERE and HOW MANY mid-song scene-change cues generation ("⟳ Generate" on the Timeline\'s Trigger bar) produces for a song. None of this ever touches a hand-placed trigger.',
          'The placement rule, per candidate moment: first look for the nearest bass-energy edge (a jump up or down, or a real pause and its resume — the same signal the music-analysis test bed\'s "Rhythmic edges" lane shows) within "Transition edge window" beats. If one is found, the cue moves there. If not, it falls back to the nearest downbeat within one beat ("Snap generated cues to beat", above). If neither, the cue stays at its raw section-boundary time.',
          '"Transition edge window" (1–16 beats, default 8): how far a cue may move to reach a rhythmic edge. Wider catches more real edges but can also drag a cue further from where the section detector originally placed it.',
          '"Transition edge sensitivity" (0.2–1.5, default 0.5): how big a bass-energy jump has to be, relative to the surrounding music, before it counts as an edge worth moving onto. Lower catches smaller jumps; higher only the most dramatic ones.',
          '"Transitions per song" (6–40, default 12): before placement, generation keeps only the strongest candidates by bass-energy step size — his own request, to bring the count closer to how many transitions he marks by hand rather than several times that. A song with fewer candidates than this number is unaffected.',
          'The music-analysis test bed\'s "Generator: Preview" lane (Test Bed page) reflects this same rule live at whatever Window/Sensitivity/Direction you drag there, before you touch the room settings — see "Tuning the room\'s automatic transitions" for that page.',
          'Re-run "⟳ Generate" after changing any of these to re-place already-generated cues under the new settings — it updates the same cues in place.',
        ],
      },
      {
        id: 'live-energy',
      ...
      },
      {
        ...
        keywords: 'force scene hold pin lock stuck stay same scene forever pick redirect reassert immediate now activate toggle recent recently used',
        body: [
          'The power button (⏻) turns the pin on/off — green and lit means it\'s on. Turning it on (or picking a different scene while it\'s already on) fires that scene IMMEDIATELY — you don\'t have to wait for a song transition or a trigger to land. On top of that, it also holds: every scene the room would otherwise pick automatically from then on — a sequencer roll, a trigger firing, or the automatic change on a song transition — fires the pinned scene instead, at the intensity/colours that pick would normally have used.',
          'While it\'s on, the panel grows to hold both a searchable list of every scene AND a "Recent" row of the 6 scenes you\'ve pinned here most recently, newest first — a shortcut for a fast re-pick mid-show, so you don\'t have to type/scroll to find the same handful of scenes over and over. A long search list scrolls inside its own box, so it is never cut off by the panel. A recent button and the search box always agree on what\'s pinned; tapping either does the exact same thing. Fewer than 6 recent picks just means fewer buttons — nothing is padded out.',
          'If nothing happens, look for the badge directly under the Force Scene toggle, at the top of the panel above the search box — it always says why: "fired" names the scene that just activated; "not fired" means nothing was pinned yet, or the pinned scene no longer exists.',
      ...
      },
      {
        ...
        keywords: 'force colour color set group pin lock hold stay same palette stop changing freeze rotation gradient drift journey toggle recent recently used',
        body: [
          ...
          'The power button (⏻) turns the pin on/off — green and lit means it\'s on. Turning it on (or picking a different colour set while it\'s already on) applies those colours to the room IMMEDIATELY — you don\'t wait for the next scene change or flare. From then on it holds: every colour the room would otherwise pick for itself wears the pinned one instead. That covers a scene fire\'s colours, the sequencer\'s own colour roll, a flare\'s colour jump, and a trigger that selects a colour set. The slow colour drift around the wheel stops walking too, and stays where the pin put it.',
          'While it\'s on, the panel grows to hold both a searchable list of every colour set/group AND a "Recent" row of the 6 you\'ve pinned here most recently, newest first — a fast re-pick mid-show without typing or scrolling. A long search list scrolls inside its own box, so it is never cut off by the panel. A recent button and the search box always agree on what\'s pinned; tapping either does the exact same thing. Fewer than 6 recent picks just means fewer buttons.',
          ...
          'If nothing happens, look for the badge directly under the Force Colour toggle, at the top of the panel above the search box — it always says why: "applied" names the set that just landed (and, for a group, which member it rolled); "not applied" means nothing was pinned yet, the pinned card no longer exists, or a pinned group has no usable member left (every one of them disabled).',
    ...
    entries: [
      {
        ...
        body: [
          ...
          'WHEN IT IS USED, AND WHEN IT IS NOT — there are three answers and two of them are "nothing". (1) GATE: the half that actually bounds the buffer is not live, so the reader can only see part of the picture and publishes the floor rather than a measured delay; applying a floor as though it were a measurement would push the lights 300–500 ms BEHIND the sound, so nothing is applied. That is where things stand today. (2) LOCK: this song has an audio lock, and that lock listens at the speakers — so it is already following this buffer, and adding the number on top would correct the same milliseconds twice. (3) APPLIED: no lock on this song, so nothing else is accounting for it, and the difference since the last calibration is added to when effects fire. The line always says which of the three it is.',
          'WHAT GETS ADDED IS A DIFFERENCE, never the whole number. The first good reading becomes the reference, and only movement away from it changes anything — so the day this starts being used, nothing about your room changes, and your A/V sync lead reads exactly the same before and after. Re-calibrating the A/V sync lead re-sets that reference, because that measurement already took the buffer as it stood.',
          'ONE KNOWN LIMIT, worth knowing rather than discovering: on a song that HAS an audio lock, a sudden drain is only corrected as fast as that lock re-checks itself (a couple of seconds, several times over, and only twice per play). On a song with no lock the step is used the moment it arrives. Closing that gap is separate work and is not done yet.',
          'YOUR A/V SYNC LEAD IS NEVER TOUCHED by any of this — not its value, not its meaning, not its limits. Everything is read and logged whatever the answer, which is how the drift picture gets built.',
      ...
      },
      {
        ...
        keywords: 'drift alarm audio chain latency session offset moved snapclient common shift level step ramp stable reversal anchor reanchor re-anchor',
        body: [
          'The drift line above the lock table watches for the whole audio pipeline moving — an audio-chain latency change (a snapclient fault, an audio-routing shuffle) shifts EVERY song\'s timing by a similar amount, and the per-song offsets quietly re-learn it one play at a time, so nothing else makes it visible. Every play counted here is locked and at/above the same quality floor a disk save requires — an unlocked or near-zero-Q play is not evidence of a song\'s normal state, so it never enters the reading.',
          'The number shown is a LEVEL: each gated play\'s winning offset minus that same song\'s own FIXED anchor, never a difference against a rolling window. A song\'s anchor is the median of its gated plays from the anchor era — the oldest four days of lock history on record when anchors were first taken; those plays are kept in their own small store as they arrive, so the lock history dropping old plays never moves an anchor, and a song first heard after those four days has no anchor and never moves this number. The anchor era never moves on its own: adopting a new normal (say, after a real audio-chain change you have decided to keep) is a deliberate re-anchor — preview it with GET /api/lock-history/drift/reanchor-preview?start_at=…, then POST /api/lock-history/drift/reanchor with start_at, who (by), why (reason) and confirm: true. Every re-anchor is logged and kept with the whole era it replaced, and the latest one shows in the reading\'s tooltip. The chip beside it compares the level with the previous session that had one: RAMP (moved 0.2–2.5 s the same way as the last move, or with no earlier move to compare), REVERSAL (moved 0.2–2.5 s the opposite way — several reversals in a row is scatter, not a trend), STEP (jumped 2.5 s or more, faster than 1 s a day — about twice the fastest ramp measured), STEP OR RAMP? (a jump that size across a gap long enough that a ramp could also explain it — two readings days apart cannot tell the two apart), or STABLE (within 0.2 s) — so a real one-time step reads as a step, not as scatter with the sign flipped. A muted "legacy" figure (a sliding 1.5–21-day-old baseline) is kept in the tooltip for continuity, but it lags a real change by design and is no longer what drives the alarm.',
          'It alarms when the session level passes ±1.5 s — deliberately before the ±3 s stale-offset error where the lock search starts failing outright, so there is real headroom to fix the audio chain while locks still land. The small trail on the right shows recent sessions\' levels oldest → newest (⚡ marks one tagged as a step). A session needs at least 3 gated plays before it can drive the alarm; first-ever plays have no anchor and never move this number. SPECTRA\'s own trigger clock rides the same shape_offset this drift is bending, so an alarm here is also why flares feel late.',
        ],
      },
      {
        id: 'timing-pipeline',
  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        ...
        keywords: 'generator stored preview rhythmic edges bass up down gap stop resume window sensitivity direction strongest N transitions per song use as room default reference set soy peor contra dopamine el apagon tuning loop transition alignment beat',
        body: [
          ...
          'Three controls appear next to the tolerance slider whenever a Rhythmic-edges lane OR the Generator: Preview lane is selected in either engine slot: Window (how many consecutive quiet beats count as a real pause, 1–16, default 8), Sensitivity (how big a bass jump has to be to count, 0.2–1.5, default 0.5), and a Direction toggle (Both / Up / Down) that restricts the lane to bass-in-and-resume ("Up") or bass-out-and-stop ("Down") edges only. Dragging or toggling any of them recomputes the lane and its score live, the same way the tolerance slider does — nothing is written until you push a mark to your real triggers.',
          'A fourth slider, "Transitions per song" (the "strongest N" density cap, 6–40, default 12), appears only for the Generator: Preview lane — Rhythmic edges just detects edges, it has nothing to rank or trim. It keeps only the N strongest candidates per song by bass-energy step size, which is what keeps the room from changing scene far more often than you mark it yourself.',
          'Since the edge-then-downbeat rule became the room\'s own placement rule (see "Transition placement rule" on the room controls bar), Generator: Preview reflects it too: it shows exactly what "⟳ Generate" would produce right now at whatever Window/Sensitivity/Direction/Transitions-per-song you have dragged here — the room\'s own snap-to-beat toggle stays at its live setting, since it has no test-bed-only copy.',
          'The three sliders start at whatever the room is already using, so opening this page never shows a false difference. "Room\'s current default" shows what the room is actually using right now for Window/Sensitivity/Transitions-per-song, and is highlighted whenever the sliders above no longer match it. "Use as room default" writes only whichever of the three you actually moved away from the room\'s value — the same fields shown on the room controls bar\'s "Transition placement rule" section — after a confirmation naming exactly what will change; a slider you never touched is left exactly as the room already had it, and dragging a slider never writes anything by itself, only pressing this button does.',
          'Below that, a "reference set" row scores the current sliders against four of your own songs at once (the same one-beat-tolerance methodology used to tune the rule in the first place) — Soy Peor / Contra / Dopamine / El Apagón — so you can see how a change moves ALL of them, not just whichever song you happen to have open. It recomputes live on every drag, same as everything else on this card.',
    ...
    entries: [
      ...
      },
      {
        ...
        body: [
          ...
          'THE SAME PROMISE WHEN THE CAMERA IS NOT A PHONE. A fixed camera on the kiosk can take this measurement instead (see "Flash pattern vs passive" below): it reduces its own microphone and camera to the identical two number streams on that machine and sends only those — no audio, no frames, nothing else. The stored record then names which machine measured it, so a kiosk run and a phone run are never the same row.',
        ],
      },
      {
      ...
      },
      {
        ...
        body: [
          ...
          'A live stream rather than a one-shot: the phone keeps streaming while the page is open, and the estimate refreshes every second over the most recent window.',
          'A FIXED CAMERA CAN DO THIS INSTEAD OF YOU HOLDING A PHONE, and it speaks the same messages — no page, no permissions, no standing still. It is run from the kiosk\'s command line rather than from here (firstmate has it), it uses that camera\'s own built-in microphone so the two stand in one place, and the answer, the statement and the refusals are identical because the server correlating it is the same code. One difference is worth knowing: a browser can sometimes tell SPECTRA exactly when a frame was captured, and a fixed camera cannot — so its statement carries a slightly wider bound on the camera side. Compare two runs from the SAME camera rather than a kiosk run against a phone run.',
        ],
      },
      {
        id: 'av-sync-apply',
  ...
  },
  {
    ...
    entries: [
      ...
      {
        id: 'room-members',
        title: 'Removing a carrier, and sitting one out',
        keywords: 'remove from room delete carrier device deselect select sitting out membership participation skip exclude pool re-add picker',
        body: [
          'Each carrier in the room has two controls, and they do different things. "Remove from room" takes it out of THIS room: it leaves the list, and what was measured for it in this room goes with it, because a footprint is a picture of what that carrier does in this room. Nothing else about it changes — its settings, its timing, and its membership of any other room are untouched. It is not a device delete; there is no device delete anywhere in this app. Add it back from the picker above and map it again.',
          'The Selected / Deselected toggle sits a carrier OUT while keeping it in the room. A deselected carrier keeps everything it has measured; it is simply skipped when you map the room — so a run neither lights it nor overwrites its footprint — and it is not offered to room effects. Press it again and it takes part exactly as before, with nothing to re-measure.',
          'Room effects have their own per-effect fixture chips, and the two layers stack in one direction: the room decides what is OFFERED, and the effect chips choose among what the room offers. A carrier deselected in the room is not driven even by an effect that names it, so "sitting out" means one thing everywhere rather than something a forgotten effect can quietly override.',
      ...
      },
      {
        ...
        keywords: 'unattended overnight queue automatic capture client headless laptop webcam v4l2 cron batch declared list runs no human remote start scope scoped take which lights fixtures room carriers other rooms bathroom left alone whole house widen',
        body: [
          ...
          'A QUEUE NEEDS THE CAPTURE CLIENT, not a page left open. Everything a queue runs is calibration-grade, so a browser session — however healthy and however well locked — cannot run a single item of one; the queue waits for the client and, if it gives up, says which camera was actually there rather than "no session". See "This page aims the camera".',
          'AN OVERNIGHT RUN THAT TAKES THE ROOM ITSELF ONLY TAKES THE FIXTURES IT DECLARED. The queue names rooms, a room names its carriers, and a carrier names fixtures — so a night mapping the living room brings up the living room\'s fixtures and leaves every other one in the house completely alone: never activated, never streamed to, and never switched off when the room is handed back. If the declaration cannot be placed against your fixture config — an unknown room, a room with no carriers chosen yet — the take widens back to the whole room rather than guessing narrow, and the night\'s own record says which of the two happened and why.',
      ...
      {
        id: 'browser-is-a-viewfinder',
        title: 'This page aims the camera; the capture client measures with it',
        keywords: 'browser viewfinder aiming demoted refuse calibration grade native capture client phone tab cached stale remote control which camera measures gain read back echo auto exposure re-adapting september 1st',
        body: [
          'The Rooms page keeps everything it was ever good at — the live preview, Start camera, the two axis taps, every result and every map on it, and ALL the run buttons. What it no longer does is BE the camera a measurement is taken with. Press Start mapping, a commissioning pass or the exposure comparison here and the run happens on whatever machine is running the capture client: you press it here, that machine measures it.',
          'THE REASON IS THE CAMERA, NOT THE CODE, and it cost a whole evening on 1 September 2026. A browser cannot hold a camera to a known state. Your Brio exposes NO gain control at all through one. The read-back repeats whatever was ASKED for rather than what the sensor did — three integration times a factor of twenty apart were all accepted, all agreed, and the measured light never moved. And its automatic exposure keeps re-adapting underneath, which is what made the picture look good and then go very dark on its own. Every number a run took under that would be a statement about the camera\'s mood, and — this is the dangerous part — nothing downstream could tell the difference.',
          'AIMING NEEDS NONE OF THAT. Pointing a camera is a live picture and your judgement; it does not care whether the sensor obeys its exposure control. So aiming stays here and stays first-class, and it is the reason the preview was never touched.',
          'A run pressed with only a browser session connected is REFUSED BY NAME, before anything happens to the room, with a sentence saying exactly this and the one next step: start the capture client on the machine beside the camera — `python -m spectra.capture_client` — or leave its boot service running, then press the same button again. The button is never greyed out for this reason: a disabled button explains nothing.',
          'The page also says WHOSE CAMERA a run will use, above the button, so two devices in one room is never ambiguous. And note that only one session is held at a time: starting the camera HERE takes it from the capture client while you aim, and the client takes it back within a few seconds of Stop camera.',
          'One more thing this closes, without any machinery of its own: a browser tab sitting on cached code can no longer silently run a calibration with old capture logic, because a browser cannot run a calibration at all.',
        ],
      },
      {
      ...
      },
      {
        ...
        keywords: 'map run sync dark black white settle capture seconds held restore revert abandon refuse exposure unseen no light seen pose missing summary brightness dim turned down bri firmware weight faint stays dark between ceiling too long minutes',
        body: [
          'Press "Map this room" and THE ROOM GOES DARK ONCE, at the start, and STAYS DARK until the run finishes. Each fixture in turn comes up full white for about two seconds while the camera watches, then goes back to black — the show does not come back in between. About four seconds per fixture. Hold the phone still for the whole run — every footprint in a map is only comparable to the others taken from the same position.',
          'It used to hand the show back between every fixture, and that was wrong twice: it looked like the lights kept flicking back to the music, and — worse — the show fading out of the fixtures landed in the NEXT fixture\'s dark reference and subtracted its light away. Staying dark closes that.',
          'It runs on the same held-room machinery every preview in this app uses, so the room is snapshotted before the first fixture goes dark and handed back afterwards. A dropped phone, a closed tab or a SPECTRA restart mid-run all land in that same recovery — your show comes back on its own, without anything having to be pressed. Stopping is YOUR press; the run never lets go of the room on its own part-way through.',
          'Because the whole run is one held room, it has its own time limit, worked out from how long the plan says it will take. The plan line shows both before you press: how many pieces, and how long the room may be held to do it. A run that would need longer than one hold is allowed to last is REFUSED there, with nothing written — it never quietly maps fewer pieces than it listed, and it never changes your "Map in" choice to make itself fit. Map part of the room at a time instead.',
          'A run measures with the CAPTURE CLIENT, not with this browser — you press the button here and the machine beside the camera takes the readings. Press it with only a browser session connected and the run is refused by name, before anything happens to the room, with the reason and the one next step. "This page aims the camera" says why.',
          ...
          'A fixture reported "clipped" was too bright for the camera at that exposure — its SHAPE is still good, its weight understates it.',
          'BEFORE the room goes dark, the plan reads each fixture\'s OWN brightness — the level set on the fixture itself, not an effect\'s — and says so loudly if one is turned down. That level scales everything the fixture emits, so a map measured with it low measures your dimmer instead of your room: it is what made the first map\'s weights come out about ten times too small. The run now takes each fixture to full for the seconds it is being photographed and puts your own level back afterwards, including if the run stops part-way. If it ever cannot put one back, it says which one.',
          'Every footprint is shown with its WEIGHT beside its picture, and one that is very faint against the rest of the room says so. The picture is scaled to its own brightest point, so a footprint holding almost no light still draws a convincing shape — the weight is the part that cannot be faked, which is why it is never hidden.',
          'A piece marked "no light seen from this pose" lit perfectly well and the camera simply could not see it — the far side of a television wrap, or spill outside the frame. That is a FACT about where the phone was standing, not a fault: the piece is recorded as such rather than vanishing from the map, so the summary reads "14 mapped, 8 unseen from this pose" instead of quietly saying 14. Photograph the room again from somewhere those pieces are visible and they fill in.',
          'If the lights are released to Home Assistant, or a handover is in flight, mapping says so and tells you to take the room back on the ownership bar — it never turns that into an error code. Lose the room part-way through and the run stops there as a stated partial: everything already measured is KEPT, so taking the room back and pressing again only does the rest. The same is true of a run that reaches its own held-time limit.',
        ],
      },
      {
        id: 'camera-settings',
        ...
        body: [
          ...
          'THE LEVERS BELONG TO THE CAPTURE CLIENT. A browser can be asked for an integration time and will say yes without the sensor doing anything about it, and it cannot be asked for gain at all — which is exactly why a calibration-grade run refuses a browser session outright. See "This page aims the camera".',
          ...
          'A LONG INTEGRATION TIME IS NOT FREE, and the run says so rather than quietly costing you frames. A sensor collecting for a fifth of a second cannot deliver more than five frames a second, whatever anything else asks for — and every capture averages whatever ARRIVED in its window, so a long exposure with the window left alone would silently average one frame instead of four. The capture windows are widened to keep the frames, the time estimate you are shown prices the WIDENED run, and an integration time so long that no legal window could average enough frames is refused before the room goes dark. That applies to a MAP, where the integration time is still yours to choose. A COMMISSIONING run is bounded — read on.',
          'A COMMISSIONING RUN NOW USES A SHORT EXPOSURE, AND WILL SHORTEN A LONGER ONE YOU ASK FOR. On 9 September 2026 the self-test refused every commissioning run on the kiosk camera, at night AND in daylight, in two different ways depending on one camera control: leave it on and the same commanded exposure measured wildly different amounts of light run to run; pin it off and a long command simply never reached the sensor. Both refusals were right, and together they say something useful — that camera is honest at the SHORT end of its exposure range and is not honest above it. The line between them is a control that lets a camera DROP ITS FRAME RATE to collect light for longer than one frame lasts, and a camera renegotiating its own timing in the middle of a reading is exactly what a commissioning decode cannot survive.',
          'So a commissioning run holds the exposure to a fraction of one frame interval — worked out from the frame rate the camera itself reports, not guessed — and turns that control off for the run, putting your own setting back afterwards. Ask for a longer one and the run uses the longest it can hold and TELLS YOU, in the notes, with both numbers. Ask for a shorter one and it is used as-is. Ask for nothing and it commands the short one rather than leaving the camera on whatever it had drifted to. A camera that never reported having either control is left completely alone.',
        ],
      },
      {
        id: 'lever-self-test',
        ...
        body: [
          ...
          'IT ONLY APPLIES TO THE CAPTURE CLIENT, and a browser session never reaches it: since the browser was demoted to a viewfinder, a calibration-grade run on one is refused earlier and for the broader reason — a browser cannot pin the camera at all, so there is nothing to self-test. See "This page aims the camera". No verdict on a browser session therefore never means a failed one.',
          'A STAMP IS NOT A PHOTON, and that is the other half of this. On 2 September 2026 this test met a real camera for the first time and refused it with two IDENTICAL commands measuring ten-thousand-fold apart — and the lever was fine. The capture client reads its pixels through a pipe that holds whole frames: measured, up to nineteen of them, nearly four seconds at five frames a second, and it fills the first moment the client is busy with something else and never gives the depth back. Four seconds is longer than a whole capture, so a lit window can land entirely on frames taken while the room was still dark. The client now throws away everything queued behind the newest frame — which costs about twenty milliseconds — and pays off a change of exposure in frames before it reports one, because a sensor takes a few frames to show a new integration time however fresh the pipe is.',
          'So if this test ever refuses with readings that disagree wildly rather than being flatly dead, the FIRST thing to check is the build on the camera machine, not the camera. A client that makes the freshness promise says so when it connects; one that does not is named in the refusal, and "Is that machine there?" shows which build each host is running. Nothing refuses on the build alone — it is a build\'s silence, not a measurement — but it is the cheapest thing to rule out and it saves an hour spent looking at a camera that was working the whole time.',
          ...
          'WHERE THIS TEST MEASURES CHANGED ON 9 SEPTEMBER 2026, AND HOW HARD IT IS DID NOT. Both of its commands are now inside the range the camera can hold steady — see "The camera: how big a frame, and the exposure levers" — and the control that lets a camera drop its frame rate is turned off for the three captures and handed back afterwards. Nothing about the judgement moved: it still commands two DIFFERENT exposures, still repeats one of them, and still refuses on exactly the same drift and the same lack of response. If anything it is harder to pass now, because there is less light in every reading — which is why the fixture is taken to full brightness for the three captures and put back to your own level afterwards.',
          'AND IF THERE IS STILL NOT ENOUGH LIGHT, IT SAYS THAT. A camera that measures nothing at both commands has always had two explanations — this pose sees none of that fixture, or the exposure control is dead — and there is now a third, which on a bounded camera is often the likeliest: there may simply not be enough light for the time this camera can hold. The refusal names all three rather than picking one, because a dead control and a dark corner must not read as each other. What it will never do is lower the bar to pass.',
          'A SETTLE TUNED FOR A FAST STREAM IS NOISE ON A SLOW ONE, found 23 September 2026 when a kiosk camera held at 1920x1080 came back "no response" on a sequence that had passed days earlier on a smaller frame size. The sensor was obeying its exposure control the whole time — measured directly, ten out of ten commands — but every change took up to two seconds to reach the delivered frames, longer than the fixed wait this test used to give a dark or lit reference before trusting it. So this test now MEASURES that delay itself, once per run, before commanding anything: it steps its own lamp from black to lit once and watches how long the delivered picture takes to show it. If that delay is inside what the protocol can absorb, the dark and lit waits are stretched to cover it and the run proceeds; if the delivered picture takes too long to show ANY change — past four seconds — the run refuses by name rather than measuring through a reference that would still show the previous moment. Either way you can read the measured delay on the result.',
          'THIS TEST CAN ALSO BE TOLD WHICH FIXTURE TO PROVE THE CAMERA WITH. Ordinarily it lights whichever fixture the run itself would measure first — right for most poses, wrong for one where that fixture is a faint smear from where the camera actually sits. A calibration can name a different, more visible fixture for this test alone, without changing which fixtures the run itself maps.',
        ],
      },
      {
        id: 'camera-host',
      ...
      },
      {
        ...
        body: [
          ...
          'IT RUNS ON THE CAPTURE CLIENT like every other measurement, and here that is doubly obvious: half of what it compares is GAIN, which a browser cannot reach at all. Pressed with only a browser session connected it is refused by name — see "This page aims the camera".',
    ...
    entries: [
      ...
      },
      {
        ...
        body: [
          ...
          'A carrier deselected on the Rooms page is never driven here, even by an effect that names it: the room decides what is offered, the chips choose among it. See "Removing a carrier, and sitting one out".',
