  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        ...
        keywords: 'transition placement rule edge window sensitivity density transitions per minute rate strongest per song beat bass energy generator generated cue mid-song align alignment R3 intensity scale mark',
        body: [
          'Three settings in the Scenes panel (room controls bar, above "Snap generated cues to beat"): "Transition edge window", "Transition edge sensitivity", and "Transitions per minute". Together with the snap toggle, they decide WHERE and HOW MANY mid-song scene-change cues generation ("⟳ Generate" on the Timeline\'s Trigger bar) produces for a song. None of this ever touches a hand-placed trigger.',
  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        ...
        keywords: 'generator stored preview rhythmic edges bass up down gap stop resume window sensitivity direction transitions per minute rate use as room default reference set soy peor contra dopamine el apagon tuning loop transition alignment beat intensity scale mark',
        body: [
          ...
          'A fourth slider, "Transitions per minute" (1–30, default 8), appears only for the Generator: Preview lane — Rhythmic edges just detects edges, it has nothing to rank or trim. This is a RATE: the per-song total is this number times the song\'s own length in minutes, scaled by that song\'s intensity-scale factor (the same one "Mark" edits, next to ⚡ Energy) — a hyped or hand-marked track earns more transitions than a mellow one at the same rate. Generation keeps only that many strongest candidates per song by bass-energy step size, which is what keeps the room from changing scene far more often than you mark it yourself.',
          'Since the edge-then-downbeat rule became the room\'s own placement rule (see "Transition placement rule" on the room controls bar), Generator: Preview reflects it too: it shows exactly what "⟳ Generate" would produce right now at whatever Window/Sensitivity/Direction/rate you have dragged here — the room\'s own snap-to-beat toggle stays at its live setting, since it has no test-bed-only copy.',
          'The three sliders start at whatever the room is already using, so opening this page never shows a false difference. "Room\'s current default" shows what the room is actually using right now for Window/Sensitivity/Transitions-per-minute, and is highlighted whenever the sliders above no longer match it. "Use as room default" writes only whichever of the three you actually moved away from the room\'s value — the same fields shown on the room controls bar\'s "Transition placement rule" section — after a confirmation naming exactly what will change; a slider you never touched is left exactly as the room already had it, and dragging a slider never writes anything by itself, only pressing this button does.',
