    ...
    entries: [
      ...
      },
      {
        ...
        body: [
          ...
          'Fish never fade out. Whenever fish have to go — the population shrinking, the lull, the drop\'s extra rush fish, or a scene change away from Fish (the crossfade) — they DISPERSE: each keeps its full brightness and swims out of the view, fast enough to be gone in time (the old fade\'s 1.2 seconds for an ordinary departure, the first ~60% of a crossfade), and only once it has fully left is it removed. A scene change into Radial is the one exception: the fish still gather into Radial\'s bloom, as before.',
          'Swim Burst (the Fish Swim Burst flare) makes every fish dash — about three times its speed — and beat its tail several times faster, for the flare\'s own length. It is an ordinary momentary flare: its hold (300ms) is how long the burst lasts, and its trigger offset (−100ms: negative means earlier) is how far before the trigger it starts. Ask Sonic to change either — e.g. "make the fish burst 400ms, starting 50ms early". It sits in each flare band\'s Shape lane, so each flare picks the burst OR Reverse, never both. Because a band fires all its flares together, the −100ms start moves that whole band\'s flares 100ms early.',
