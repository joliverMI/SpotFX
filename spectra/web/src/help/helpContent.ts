  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        ...
        body: [
          'OPENING THIS PREVIEW CHANGES YOUR REAL LIGHTS. Your trigger engine pauses the instant you open it — if a song is playing, its scene changes, other flares, and charge/lull/drop moments all go silent for as long as the window stays open, whether or not this kind has fired yet. Opening does NOT fire anything by itself any more: the scrub ruler loops on its own real-time clock, and every time the white playhead reaches the accent-coloured "start" line, this card\'s own scene fires onto your fixtures and this one flare kind fires on top of it for real — then it holds there until the NEXT lap reaches "start" again, over and over, for as long as the window stays open. This is a TRUE SIMULATION of a trigger crossing, not a single flash: the first fire waits for the mark exactly like every fire after it. Closing the preview (or navigating away) reverts your room to exactly what it was showing before you opened it, and your live show resumes from wherever it would otherwise be. Two separate releases protect you here, and they are not the same guarantee: if you close the browser tab or lose connection instead of pressing close, the app notices on its own and reverts anyway — typically within about 17 seconds of your last moment on the page. Separately, and no matter what — even if the tab stays open and keeps checking in the whole time — a preview can never hold your room for more than 3 minutes total; past that it releases itself automatically and tells you so right in the panel, whether or not anything else ever notices. The first number is how fast an abandoned preview lets go; the second is the hard ceiling on how long any single preview can run at all.',
