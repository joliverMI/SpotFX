    ...
    entries: [
      ...
      },
      {
        ...
        body: [
          ...
          'THE MARKS HERE ARE NOT DRAGGABLE, and that is deliberate. Each mark is its band\'s ANCHOR — the earliest offset any flare kind attached to that band authored — while every kind in the band keeps its own offset, so a drag would have to pick one kind to write it to, and picking for you would be inventing an answer. Retime a kind from its own ▶ Preview on the Flares or Charges/Lulls/Drops tab and this ruler shows the result: each class\'s mark moves to its band\'s anchor, which is where that fire begins, and the kinds inside it land at their own moments relative to that mark — a kind authored later than the anchor fires that much after it, here and in your show alike.',
