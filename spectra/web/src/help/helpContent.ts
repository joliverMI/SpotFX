  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        ...
        keywords: 'dark mode light display mode lock dark_lock brightness multiplier dim undim ambient color colour dark colour second colour transition pace global room bar dimmer midsong mid-song trigger fallback scene change mode transitions analysed full settings model force scene hold pin catch-up catchup ease release ramp snap read-back readback confirm confirmed unconfirmed partial straggler retry rate limit zigbee bulb name music wins precedence yielding holding partial transitioning resting state playing paused off always auto on during music auto-return three settings mode stale age verified verify honesty confirmed ago hue areas entertainment area group groups dining room which lights press hold long press tap cycle grouped button expand panel light bulb icon toggle spam delay debounce',
        body: [
          ...
          'Mode groups the Hybrid/Dark/Light display-mode control with its Light-mode colour picker and brightness slider. A short tap CYCLES the three modes (Hybrid → Dark → Light → …) — the button itself is coloured by whichever mode is current (Light shows the actual configured colour, since that\'s the point of the mode). Holding the button (~½s) opens the colour/brightness panel instead of cycling. The button\'s display updates the instant you tap — cycling never feels delayed — but the actual change is only sent to the room after a full second with no further taps, so cycling past a mode on your way to another one never spams the room with modes you were only passing through; if you keep tapping, only the mode you land on is ever applied. See "Display mode — Hybrid, Dark, or Light" below for the mode mechanics themselves.',
