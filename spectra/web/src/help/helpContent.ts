  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        ...
        title: 'A light that is dark while SPECTRA streams to it',
        keywords: 'dark fixture watch dark light off switched off on=false brightness zero unreachable dropped off network gone stopped answering not receiving live=false streaming streamed writing into the dark tv backlight tv-backlight wled strip mid-show mid song silent fault named blind spot healthy liveness devices online false healthy sweep 30 seconds 60 seconds recovered came back read back json state json info',
        body: [
          'WHAT YOU SEE: an amber strip at the top of every page — "⚠ 1 light is dark while SPECTRA streams to it" — naming the light, saying which of four things is wrong, and how long it has been that way. It appears whether or not there was ever a take-back problem, and it is a different strip from "A light the take-back had to skip" above only because the two catch the fault at different moments: that one can only speak about lights that were already dark when SPECTRA took the room, this one about a light that was fine and then went.',
          'WHAT IT MEANS: SPECTRA is pushing frames at that fixture right now, and the fixture itself says it is showing nothing. Four sentences, four different next steps: "has stopped answering" — it has dropped off the network entirely while being streamed to (power, WiFi, or the controller falling over under the stream); "answers but reports it is NOT receiving SPECTRA\'s stream" — it is up and talking but not taking the data (realtime disabled on it, or a blocked port); "is switched OFF at the fixture" — it is taking the stream and is powered off in its own firmware, so the data lands on a dark strip (something turned it off outside SPECTRA — the WLED app, a Home Assistant automation, its own timer); "its master brightness is 0" — the same, one knob over.',
          'WHY IT EXISTS: every other health reading in this app can say FINE over exactly this state, and did, twice, on the same fixture. Frames flowing proves SPECTRA\'s render loop pushed a frame, not that anything received it. The device "online" flag on the liveness endpoint is set once when the device is built and, for a WLED, essentially never clears — a fixture off the network for hours still reads online. And the take-back report only ever re-asks lights it already named. So a light going dark mid-show was invisible until someone looked at the wall. This watch reads the fixture\'s own answer back every 30 s and names it once it has been dark for a full minute — so a genuine fault shows up within about a minute and a half, and a single missed reply never accuses a working light.',
