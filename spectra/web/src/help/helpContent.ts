    ...
    entries: [
      {
        ...
        body: [
          ...
          'Saving is no longer the only thing that lands: since 2026-08-25 every path that WRITES a song\'s whole trigger list updates the firing copy too — an analysed/AI trigger import, the automatic generation that runs after a fresh capture, and the timestamp shift applied after a re-capture. Before that fix those paths wrote only the profile file, so an imported song kept firing its old triggers (or none) until an operator catch-up pass.',
