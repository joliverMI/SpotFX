  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        ...
        keywords: 'sequencer curve likelihood dwell affinity genre profile inline one-off detach save named overwrite grid thumbnail picker window scroll collapsed',
        body: [
          ...
          'The curve is picked from a SCROLLABLE GRID, not a name-only dropdown: every tile draws that curve\'s own real shape (same points, same scaling the full editor uses), so you choose by shape instead of picking blind and finding out after. The grid stays COLLAPSED — no curve is expanded — until you click a tile. Each named profile gets a tile, plus fixed tiles for "— not sequenced —", "Flat 1.0", and "Inline one-off…" (whose tile previews what it would start from — the currently-attached curve — since a not-yet-detached scene has no inline shape of its own yet). The selected tile is outlined; clicking any profile or "Inline one-off…" tile both selects it (the scene\'s attachment saves immediately, as before) and opens its curve window — flat and not-sequenced have no shape of their own to edit, so they just select.',
          'The curve window shows the full editor and lets you drag points. THE SAFETY RULE: dragging a point NEVER writes to the shared profile — it only ever updates a local, unsaved draft, and the status line switches immediately to "Editing — one-off, not yet applied" the moment you touch a control. Nothing else changes until you choose one of two explicit actions: "Apply edited one-off to this scene" (or "Detach — edit just this scene" before you\'ve touched anything) copies the curve into THIS SCENE\'S OWN inline curve only — it can never retune another scene sharing a profile, because it never writes to the shared curve store at all. "Save as named curve…" is the only action that can write a shared, named profile: type a name and Save. If that name already matches an existing curve, a warning dialogue names it, how many other scenes use it, and requires an explicit "Overwrite" click before anything is written — Cancel/Back leaves the stored curve untouched.',
          'A curve is either a named PROFILE (pulled from the grid — a shared shape; editing and applying it retunes every scene using it, shown by "shared by N scenes" and the "changes every scene using it" caption) or an INLINE one-off (this scene only, no name, nothing added to the profile library).',
    ...
    entries: [
      ...
      },
      {
        ...
        body: [
          ...
          'Pause GENUINELY drops the live connection — not just a blank display. Resume reopens it. This choice is sticky: it\'s saved, so it survives a SPECTRA restart, and it stays paused until you click Resume — it never un-pauses itself.',
          'Switching to another browser tab (or minimizing the window) auto-pauses the preview too, and switching back auto-resumes it — no click needed either way. This is a SEPARATE, temporary mechanism from your own Pause button, never saved and never something you have to remember to undo: it only ever reflects whether this particular tab is the one you\'re looking at right now.',
          'The badge always tells you which of the four states you\'re in, so you can tell at a glance why nothing\'s streaming: "live" (connected, showing real colour); "paused" (gray — you clicked Pause; stays this way until you click Resume); "idle — tab hidden" (blue — this tab isn\'t visible right now; it\'ll pick back up on its own the moment you switch to it); "reconnecting…"/"unavailable" (amber — the source is momentarily unreachable, or nobody currently owns the lights at all — e.g. a handover is in flight; the preview never restarts or wakes anything, it just waits). Your own Pause always takes priority in the badge — pausing manually and then switching tabs still reads "paused," not "idle."',
          'If it ever costs you something you\'d rather not pay while you\'re not looking at it, Pause is the answer for a deliberate stop — the tab-hidden behaviour already handles the common case of just switching away for a while.',
        ],
      },
      {
        id: 'dark-light-mode',
        title: 'Dark mode — force every background black',
        keywords: 'dark light mode display mode toggle lock dark_lock background black hard clamp shield singles category exempt restore repaint snapshot music playing deferred',
        body: [
          'The legacy global Dark/Light display-mode toggle, ported to a single Dark mode switch on the room bar. Turn it on and every device\'s background is hard-clamped to black at LedFX itself (the same dark_lock flag the legacy toggle drove) — no write path, drift glide, or scene fire can relight a background while it\'s set. Turn it off and whatever each device was showing right before you turned it on comes back, repainted from a snapshot taken the instant you switched to dark — UNLESS music is playing right now, in which case that stale snapshot is deliberately left unpainted (a grey "repaint deferred to live show" badge says so) and the room\'s own automatic show repaints it on its next natural fire instead. Forcing an old still frame back over a room that should be tracking live music is the same mistake as freezing the room under Ambient mid-song — the switch still clears the lock either way, it just doesn\'t also impose a stale look while something live is about to replace it anyway.',
          'This is a bool, not legacy\'s three-way Default/Dark/Light cycle: legacy\'s "Default" existed to defer to lower per-node dark/light overrides (a trigger, a scene, a colour card) that SPECTRA never rebuilt (retired — see the Light Mode Chooser was never a SPECTRA feature). With nothing left to defer to, "off" already means exactly what "Default" and "Light" both meant in legacy: nothing forced.',
          'Devices in the "Singles" category (legacy\'s own default) always keep their own look regardless of this switch — SPECTRA never locks or unlocks them. Additional exempt categories/virtuals are settable via the room-controls API (dark_light_shield_categories / dark_light_shield_virtuals), not yet a bar-level control.',
          'Composes with Ambient\'s three settings, not a simple on/off: whenever a Hue device is actually frozen right now (Ambient "On during music" unconditionally, or "Auto-return" while confirmed quiet), that device is driven by direct bridge REST, bypassing LedFX entirely — Dark mode\'s dark_lock clamp has no visible effect on it. It DOES still change which Ambient colour that frozen device holds, though — see "Ambient\'s dark-mode colour" below; dark mode never bypasses Ambient, and Ambient never blocks dark mode, dark mode changes what Ambient holds. The moment a device isn\'t frozen (Ambient "Off", or "Auto-return" while music plays), it\'s rendered by LedFX like any other device and responds to Dark mode\'s dark_lock clamp normally.',
          'A red badge next to the switch names any device that didn\'t confirm the requested lock/unlock state after a live read-back from LedFX — checked, never assumed.',
        ],
      },
      {
