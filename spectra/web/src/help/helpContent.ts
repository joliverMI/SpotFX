  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        ...
        keywords: 'sequencer curve likelihood dwell affinity genre profile inline detach promote share grid thumbnail picker',
        body: [
          ...
          'The curve is picked from a GRID, not a name-only dropdown: every tile draws that curve\'s own real shape (same points, same scaling the full editor below uses), so you choose by shape instead of picking blind and finding out after. Each named profile gets a tile, plus fixed tiles for "— not sequenced —", "Flat 1.0", and "Inline one-off…" (whose tile previews what it would start from — the currently-attached curve — since a not-yet-detached scene has no inline shape of its own yet). The selected tile is outlined.',
          'A curve is either a named PROFILE (pulled from the grid — a shared shape; editing it retunes every scene using it, shown by "shared by N scenes" and the "changes every scene using it" caption) or an INLINE one-off (this scene only, no name, nothing added to the profile library). Pick "Inline one-off…" to save a curve directly to this scene without creating a profile.',
