  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        ...
        keywords: 'brightness multiplier dim undim ambient color transition pace global room bar dimmer midsong mid-song trigger fallback scene change mode transitions analysed full settings model force scene hold pin',
        body: [
          ...
          'Ambient: on/off + a colour swatch (legacy ledfx_ambient / ledfx_ambient_color actions). A real takeover: every live Hue device in the room freezes its entertainment stream (so the bridge falls back to normal REST control) and holds at the chosen colour, full brightness, over a short fade. Non-Hue devices (WLED etc.) keep running their normal reactive show, same as the legacy behaviour this ports. Turning it off fades down and unfreezes — the room\'s live scene picks back up on its own, no extra step. If SPECTRA isn\'t currently driving the lights, or there\'s no live Hue device in the room, a small grey "ambient: dark" / "ambient: no-hue-devices" badge shows next to the swatch — the switch still saves, but nothing changed live. A red "ambient: failed" badge means SPECTRA tried and every Hue device rejected it (bridge unreachable, etc.) — the switch saved, but the room may not actually match it; check the bridge. Dinner Party (the other legacy room-MODES behaviour) is still a separate, later build.',
