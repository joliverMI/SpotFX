"""S2 evolution-engine observability + the dark event injector.
  ...
  POST /api/engine/event  — inject one response event at a chosen intensity
      ...
      input. It settles immediately: a band's staggered kind batches run
      right after the fire, in due order (reported as `kind_batches`), then
      every pending release is flushed.
