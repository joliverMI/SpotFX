...
OPERATIONS: dict[str, SonicOperation] = {
    ...
    "set_flare_kind": SonicOperation(
        ...
        instructions=(
            ...
            "positive = later, 0 = on the mark (-60000..60000). Each kind "
            "in a band lands at its OWN offset: the band's fire begins at "
            "the smallest offset among its enabled kinds, a kind at 0 "
            "included, and every other kind waits the difference. So "
            "[0, +300] fires the 0 kind on the mark and the other 300ms "
            "later; [-100, 0] fires one 100ms early and the rest on the "
            "mark. On a charge/lull/drop band that fire start is also where "
            "the phase choreography begins, so it only starts late once "
            "EVERY enabled kind in the band is positive. Like "
