...
def _layer_systemic(offset_ms: int, quality: float, source: str) -> tuple[int, float, str]:
    """Add the systemic starting-offset bias on top of the per-song /
    per-Set-List resolution. A cold-start aid only — this play's own xcorr
    re-lock overrides it via apply_save; inert unless the learner is enabled
    and confident (see services/systemic_offset.py)."""
    try:
    ...
    async def _execute_morph_step(self, action, await_ramps: bool = False,
        ...
        for w in switch_writes:
            ...
            _HANDOFF_FAMILY = {
                ...
                "squiggles",
                "dancer",  # body points burst into the successor
            }
            handoff_pair = (
