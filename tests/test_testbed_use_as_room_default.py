...
transition_edge_sensitivity, transitions_per_minute) through the
...
def test_the_exact_use_as_default_payload_writes_only_those_three_fields():
    ...
    assert saved["transitions_per_minute"] == 6
    ...
    assert stored["transitions_per_minute"] == 6
...
def test_a_later_get_reflects_the_written_values():
    ...
    assert got["transitions_per_minute"] == 20
...
def test_out_of_bounds_values_are_rejected_not_silently_clamped():
    ...
    resp2 = client.put("/api/room-controls", json={"transitions_per_minute": 0})
