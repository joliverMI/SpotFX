...
| **The `--system` install actually REACHES that verify** | The installer's own `--system` path is run end to end against a throwaway `HOME` with `sudo` shimmed to a recorder, so nothing reaches `/etc` — and the recorded `install` line carries the exact path the installer verified. It used to generate the unit into a bare `mktemp` file, and `systemd-analyze verify` resolves a unit's TYPE from its BASENAME, so an unsuffixed name was refused ("Failed to prepare filename ...: Invalid argument") before the contents were read: a valid unit, a refused kiosk install (River, kiosk-0, systemd 257, 2026-09-07). §1b verified the generated bytes but GENERATED THEM ITSELF, which is why nothing here saw it. `check_capture_client_service.py` §3b. |
...
- **A SYSTEM unit under a REAL ROOT MANAGER.** *(2026-09-03)* The installer
  ...
  `tests/test_capture_client_service.py`). **And since 2026-09-07 the
  installer's own `--system` run is driven for real** (§3b, `sudo` shimmed
  so nothing reaches `/etc`) — §1b generated the bytes itself and therefore
  could not see that the installer's verify step was refusing every kiosk
  install over its TEMPORARY FILE'S NAME. **What is NOT proven is a root
