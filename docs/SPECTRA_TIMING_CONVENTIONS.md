...
| `band_trigger_offset_ms(scene, class, intensity)` / `_band_anchor_ms(band, declared)` | ms | Same OFFSET convention; the band's ONE anchor is `min()` over EVERY declared, enabled kind's value, zero included (earliest ask wins; FIXED 2026-09-16, spotfx-zero-offset-fires-on-mark — it used to be min over the nonzero values only, which anchored an all-positive band on its positive value and fired an untouched-0 kind late) — still what `tick()` reads to relocate the whole fire, and therefore also where a charge/lull/drop's `_drive_phase` begins: a phase band's choreography only starts late once EVERY enabled kind in it (pooled alternatives included) carries a positive offset. PER-FLARE TRIGGER MOMENT (2026-09-16): inside the fire, `_execute_band_locked` now splits attached kinds by `delay_ms = max(0, kind.trigger_offset_ms − anchor_ms)` (always >= 0, legacy `MorphLane` shape) — the delay-0 group runs inline, any other delay is a `PendingKindBatch` due at the fire's START (read before `_drive_phase` and the inline writes) + `delay_ms`, scheduled the same way a `PendingRelease` is — ONLY on a fire `tick()` already relocated by that anchor (`anchor_relocated`, i.e. `fire_response_event(via_trigger=True)` and the drop-sequence preview); a bridge flare, `on_update` and a manual test-fire run the band atomically, unmoved. Because the anchor is the true min over every declared kind, no kind's delay is ever clamped, so a kind's real moment equals its own offset (the automatic lead aside, which stays band-wide) | SPECTRA response engine, read by `tick()` for `fire_response`; per-kind split read by `ResponseEngine._execute_band_locked` | **Live** | `spectra/services/scene_response.py` (module docstring, "PER-FLARE TRIGGER MOMENT") |
...
| `audio_shape_service._TAIL_WAIT_CAP_S` (3.0) via `_capped_wait_s(wait, cap)` | s | **Not signed, and NOT a member of either family** — a plain WAIT, recorded here because it is derived from `audio_latency_ms` one row up and because its MEANING was wrong until 2026-09-09. Both capture tail-waits (`on_track_change`'s boundary-wait for the previous song's tail to reach the capture device, `_stop_and_save`'s tail-wait for the boundary audio) hold the poll loop while the audio pipeline delivers sound `audio_latency_ms` behind Spotify's own report; the cap bounds how long that block may last. **The defect: the 3.0 was the wait's ELIGIBILITY bound** (`if 0 < wait_s <= 3.0: sleep(wait_s)`), so a wait needing MORE than 3 s was skipped ENTIRELY — zero wait, i.e. exactly the cut-off tail the surrounding comment already named as the thing to avoid. `_capped_wait_s` is now the ONE definition both sites call and it sleeps `min(needed, cap)`. A cap is not an eligibility test — if you add another wait on this path, clamp it | Predecessor, capture alignment (same owner as the row above) | **Live** (predecessor-side; nothing in `spectra/` reads it) | `services/audio_shape_service.py` (`_capped_wait_s` and `_TAIL_WAIT_CAP_S`, each carrying the reasoning inline), `tests/test_audio_shape_capture_trim.py` |
...
| Fish dispersal + swim burst, effect-side: `DEPART_S` (1.2 s), `LULL_LEAK_FROM`/`LULL_LEAK_TO`/`LULL_EXIT_BY` (0.4/0.72/0.92 of `LULL_GONE_AT`), `LULL_EXIT_MIN_S` (0.3 s), `TRANSITION_EXIT_BY` (0.6 of the crossfade), `BURST_ATTACK_S`/`BURST_RELEASE_S` (0.03/0.05 s), `BOUND_BRAKE_TAU` (1/60 s) | s / phase-progress fraction / crossfade-weight fraction | Not signed. `p_dl` is an effect-clock DEADLINE a dispersing fish must be off the panel by; its speed is derived from it, never tuned. The lull fractions sit on the SpotFX-ramped `phase_progress` axis (same ~90%-then-hang caveat as Black Hole's row), converted to seconds by the progress rate read back off the effect's own phase clock. The burst has NO duration of its own: the Fish Swim Burst flare kind owns it — `hold_ms` (300) is the length and `FlareKind.trigger_offset_ms` (authored 0, OFFSET family, negative = earlier) the start, so it fires ON the trigger; a toggle computes lead 0, so nothing else shifts it. `BOUND_BRAKE_TAU` is the swim-burst boundary brake's time constant — the brake's per-frame take-back of above-cruise speed compounds per unit time (`1 - (1 - brake) ** (dt / BOUND_BRAKE_TAU)`), so it holds at any frame rate; it moves no moment | `fx/effects/fish.py` (effect-side; SPECTRA writes `phase`/`phase_progress`/`swim_burst`) | **Live** once deployed + `scripts/add_fish_swim_burst_flare.py` applied | `fx/effects/fish.py` (dispersal + swim-burst blocks near the top), `fx/VENDOR.md` #39 |
...
| `room_mapping.{DARK_SETTLE_S,DARK_CAPTURE_S,LIT_SETTLE_S,LIT_CAPTURE_S}` (0.7 / 0.5 / 0.7 / 1.5) + `room_effects.{TICK_HZ (15), TYPE_REFRESH_S (10)}` | s / Hz | **Not signed, and NOT members of either family** — recorded here so nobody later reads a light-field capture window as an offset. The four capture numbers are the light-field mapping protocol's own settle-and-average windows (a write has to LAND before the reference means anything, hence a settle either side); `TICK_HZ` is the room-effects wave's write cadence, paced against a deadline rather than by sleeping a whole period after the work, so the achieved rate matches the target and a wave's speed knob means what it says. The wave composes MULTIPLICATIVELY at the write seam and shifts no clock: it is not `av_sync_lead_ms`'s neighbour, it is `brightness_multiplier`'s. | SPECTRA room light-field | **Live** (2026-08-31) | `spectra/services/room_mapping.py`, `spectra/services/room_effects.py` (constants near the top of each), measured cost in `scripts/check_room_effect_wave.py` |
| `capture_client.camera.{DRAIN_PROBE_S (0.02), DRAIN_MAX_FRAMES (64), SENSOR_APPLY_FRAMES (3)}` + `capture_settings.regime_settle_s(exposure, fps)` | s / frames | **Not signed, and NOT members of either family** — these are PIPELINE LAGS, i.e. how long after an act the instrument can be trusted to be showing it, and they are recorded here so nobody reads one as a fire-time offset. Two different lags with two different owners. TRANSPORT: the capture client's ffmpeg pipe holds whole frames — MEASURED at up to 19 at the 320x180 map rung, 3.8 s at 5 fps, and it saturates there — so `camera.frame()` returns a frame only when nothing newer is already waiting (`newest_of`); `DRAIN_PROBE_S` is how long a probe waits before deciding nothing is buffered (an order of magnitude above the ~1 ms an already-queued frame costs, and under a 30 fps frame period), `DRAIN_MAX_FRAMES` bounds it. SENSOR: a commanded integration time reaches the sensor `SENSOR_APPLY_FRAMES` frames later, which no drain can reach — the client discards that many after a control MOVES, and the server states the same bound independently as `regime_settle_s` = (SENSOR_APPLY_FRAMES + TRANSPORT_LAG_FRAMES) frame periods at the ACHIEVABLE rate + the integration itself, paid once per commanded regime (so an ordinary map, whose exposure never moves mid-run, pays nothing). **A capture stamp is when the CLIENT read the frame out of its transport, never when the photons landed** — `MappingSession.gather` windows correctly against that stamp on the paired clock and cannot see past it, which is why the fix is the client's. | SPECTRA capture client + the light-field/commissioning capture path | **Live** (2026-09-02; needs a `spectra.service` deploy AND a camera-host pull) | `spectra/capture_client/camera.py` ("FRESH FRAMES" block), `spectra/services/capture_settings.py::regime_settle_s`, measured in `scripts/check_stream_freshness.py`, swept in `tests/test_stream_freshness.py` |
| `room_mapping.FRAME_SWITCH_WAIT_S` (4.0, a FLOOR since 2026-09-06) + `capture_settings.{CLIENT_RESIZE_BUDGET_S (15), frame_switch_wait_s(exposure, fps, lever_moved)}` | s | **Not signed, and NOT a member of either family** — it is a bound on REFUSING a client, never a delay before succeeding: `CameraNegotiation.await_frame_size` returns on the FRAME, so a working camera pays none of it. Recorded here because it is the one wait on the capture path that was picked rather than derived, and the picking cost a real evening. Asking a client for a new wire frame commands THREE acts — a pixel-pipe restart at the new size (for which the client allows ITSELF 15 s, `capture_client/camera.py::_open_at`), a re-read of every control, and, when the same `config` message names a lever, the sensor settle that lever owes (`regime_settle_s`, the row above) — and until 2026-09-06 all three shared ONE fixed 4.0 s window, so his sconce commissioning refused a camera that was working (22:44, between two runs that worked) and refused it WORSE whenever an integration time rode along. `frame_switch_wait_s` ADDS the three from the client's own numbers and `await_frame_size` takes `max(caller, derived)`, so the caller's constant is a floor and never a ceiling. | SPECTRA capture runs (map, commissioning, exposure comparison, pose fingerprint) | **Live** (2026-09-06) | `spectra/services/capture_settings.py::{CLIENT_RESIZE_BUDGET_S,frame_switch_wait_s,CameraNegotiation.await_frame_size}`, proven in `tests/test_frame_size_switch_wait.py` |
| `capture_settings.{STREAM_LAG_REFUSAL_S (4.0), STREAM_LAG_SETTLE_MARGIN (1.5), STREAM_LAG_SAMPLE_WINDOW_S (6.0), STREAM_LAG_BASELINE_S (0.3), STREAM_LAG_POLL_S (0.1)}` | s | **Not signed, and NOT a member of either family** — a MEASURED pipeline lag, one layer upstream of the row above: `regime_settle_s` prices the SENSOR's own apply delay; this prices the STREAM's own delivery delay (measured on a kiosk held at 1920x1080/5fps, 1.4-2.1 s per control write or scene change — `data/kiosk-exposure-lever-no-response/report.md`, 2026-09-23). Measured once per lever-self-test run (`lever_selftest._measure_stream_lag`, the live half; `stream_lag_crossing`, the pure half) by stepping the driven emitter black→lit and watching how long the delivered stream takes to cross halfway to its plateau. `widen_settle` folds the answer into `room_mapping.{DARK_SETTLE_S,LIT_SETTLE_S}` for that run's three regime captures ONLY (never into `regime_settle_s`, a different lag with a different owner); a lag past `STREAM_LAG_REFUSAL_S` refuses BY NAME (`mapping_refusals.LEVER_STREAM_LAG`) rather than measuring through it. | SPECTRA lever self-test (the capture protocol's own dark/lit settles) | **Live** (2026-09-23) | `spectra/services/capture_settings.py` (SIX section), `spectra/services/lever_selftest.py::_measure_stream_lag`, proven in `tests/test_capture_settings.py` + `tests/test_lever_selftest.py` |
...
| `dark_fixture_watch.SWEEP_INTERVAL_S` (30) / `FAULT_AFTER_S` (60, over ≥ `MIN_BAD_READS` = 3 consecutive dark reads) / `HTTP_TIMEOUT_S` (3) / `READ_TIMEOUT_S` (= 2×`HTTP_TIMEOUT_S`+1 = 7) | s | **Not signed, and NOT members of either family** — durations and cadences, recorded here so nobody later reads the 60 as an offset. In order: how often every fixture SPECTRA is streaming to is read back (the same 30 s tick as the activation recheck one row up); how long a fixture must read dark/not-answering CONTINUOUSLY — and across at least `MIN_BAD_READS` reads; the threshold is the PAIR, never the seconds alone — before it is NAMED on the liveness/ownership/engine-status surfaces (≤ 90 s worst case: an order of magnitude over a WLED's measured 6.2–6.4 s live-flag rise, a single lost request under a write burst, and a controller reboot); the per-HTTP-request budget that decides `unreachable`, handed to the vendored transport explicitly (`WLED._wled_request(timeout=)`) because that transport's own default is 0.5 s and an outer bound never binds; and the whole two-request read's outer bound. `probe_device_live` keeps the vendored default, so the activation gate's timing is untouched. An UNKNOWN read (lost json/state, reader crash) neither advances nor resets the clock, and a kind change does not restart it. | SPECTRA dark fixture watch (detection only — it never writes to a fixture) | **Live** | `spectra/services/dark_fixture_watch.py:198-202` (constants near the top of the module; the threshold rationale is in the module docstring) |
| `config.pretake_settle_ms()` (`SPECTRA_PRETAKE_SETTLE_MS`, default `PRETAKE_SETTLE_MS_DEFAULT` = 1500, clamped to [0, `PRETAKE_SETTLE_MS_MAX` = 30 000]) + `pretake_ping.{CONNECT,READ,WRITE,POOL}_TIMEOUT_S` (2 / 3 / 2 / 1) | ms / s | **Not signed, and NOT a member of either family** — a plain WAIT and a request budget, recorded here so nobody later reads the 1500 as a lead or an offset. The settle is how long a TAKE of the room pauses between announcing itself to River and touching its first fixture, so her snapshot watch can read the room SPECTRA found: it delays a take and never shifts one moment relative to another. 1500 is ample against her measured ~0.4 s capture and is lost inside `handover.HUE_RELEASE_GRACE_S` (5 s), which every take already spends releasing the Hue session. It is CLAMPED because it sits on the critical path of taking his room — a typo in a systemd unit must cost at most 30 s, never an hour of held-dark room — and `0` is legal and means "announce, do not wait". An UNSET `SPECTRA_PRETAKE_URL` spends none of it: nothing is sent and nothing is waited for. **The RELEASE ping (`pretake_ping.after_release`, 2026-09-07) spends none of it EITHER, at any setting, and that is the design rather than an omission**: a settle exists to give River a window on a room that is ABOUT to change, and a release has already happened — there is no race left to lose, so waiting would only hold the panic handle open while nothing was protected. `after_release` has no `sleep` seam at all, which is what stops the wait being reintroduced by analogy. The four transport budgets are tighter than `witness.py`'s (3/6/4/1) for the same reason: the witness's query overlaps a capture's own settle, where this one is spent in front of his room changing hands. | SPECTRA boundary pings (`handover.run_handover` in the take direction, `night_take.take_room`; the release ping in `release.release_room` spends none of it by design) | **Live** | `spectra/config.py` (`pretake_settle_ms`, and the two bounds beside it), `spectra/services/pretake_ping.py` (the timeouts near the top; the reasoning is in the module docstring) |
| `av_offset_ms` (phone A/V-sync instrument: `light_lag_ms − audio_lag_ms`) | ms | **MEASURED, neither family**: positive = the LIGHT reached the phone LATER than the sound it was meant to land with (lights behind/lag); negative = lights EARLIER (ahead/lead). `light_lag_ms` = (phone sees a light edge) − (server wrote it); `audio_lag_ms` = (phone hears a sound onset) − (SPECTRA's own audio hub heard it); the phone↔server clock offset cancels in the difference. A measurement of his room from where the phone stood — NOT authored, NOT applied anywhere by the build that introduced it (the number is presented for him to accept; no setting is written). To close a measured offset the engine fires lights EARLIER by `+av_offset_ms` (a LEAD-family quantity, positive=earlier) — **that translation was built 2026-08-28 and is the `av_sync_lead_ms` row immediately below**, as this row asked. The measurement itself is unchanged by it: still measure-only, still never applied without his press. **THE CAMERA NEED NOT BE A PHONE (2026-09-07)**: `spectra/capture_client/avsync_session.py` speaks the identical protocol from a fixed kiosk camera with the Brio's own microphone beside it — same arithmetic, same sign, same refusals, zero server change beyond five `hello` keys the record now KEEPS (`source`/`client`/`client_version`/`host`/`pose_name`) so a stored run says which camera took it. Its two client-side terms are the SAME ones the server already names for a phone with no `captureTime`: the camera pipeline (sensor→USB→ffmpeg→the read stamp, bound 80 ms, direction lights_look_later) and the microphone pipeline (ADC→ALSA→the read, bound 40 ms, lights_look_earlier) — reported as unknown rather than corrected, opposite in direction, and constant for one camera in one place, which is why a DIFFERENCE between two runs from that camera is worth more than either absolute figure | SPECTRA AV-sync instrument (`spectra/services/av_sync_session.py`, `av_sync_correlate.py`, UI `/avsync`) | **Live** (measure-only; pattern mode flashes the room over fx_seam and reverts — and since 2026-08-28 can be narrowed to ONE device, `PatternDriver.start(device_id=)`, so a run measures one fixture's own path; the BETWEEN-DEVICE DIFFERENCES are what `spectra/services/device_equalization.py` turns into proposed `timing_offset_ms` values, because the shared audio path cancels in the subtraction) | `spectra/services/av_sync_correlate.py` (module docstring: the algebra), `spectra/services/av_sync_session.py` (module docstring: sign + privacy), `scripts/check_av_sync.py` (simulated rooms, the number vs truth), `scripts/check_avsync_kiosk.py` (the kiosk's own reductions driving the same correlation from RAW media, both signs, with a flipped-sign and a naive-audio-stamp red control) |
...
| `effects_fire_later_by_ms` — River's published audio buffer, and SPECTRA's `known_buffer.compensation_ms()` derived from it | ms | **Positive = the SOUND is running that much further BEHIND, so effects fire that many ms LATER.** Never negated, never renamed, never shortened — the wire field, the five `RoomControlState.known_buffer_*` settings, the status key and the help topic all carry the name verbatim. **NOT LEAD family and not `offset_ms` family: it is subtracted at the show clock, on the SAME LINE as the lead it must never be added to with one sign** — `show_clock_ms = effective_position_ms + av_sync_lead_ms − known_buffer_ms` (`spectra/services/av_sync_lead.py::show_clock_ms`, still the single application point; the new argument defaults to 0, so every pre-existing caller is byte-identical). **The applied number is a REFERENCE-BASED DELTA, never the absolute buffer**: `compensation_ms = effective_value_ms − reference_ms`, the reference anchored on the first reading the apply gate accepts and re-based whenever `av_sync_lead_ms` is applied (that measurement absorbed the buffer as it stood), so going live changes nothing in his room. **LOCK-GATED, ruled 2026-09-17** — three answers, and two of them are zero: `gate` (the apply gate is shut — `floor_clamped`/not `governed`, i.e. River's bounding half is not live and the published number is the contractual FLOOR rather than a delay), `lock` (`bridge.shape_offset_ms()` is not None — the xcorr lock taps `snapcast.monitor`, MEASURED to be the monitor of the sink `snapclient` plays INTO, i.e. speaker time downstream of the snapserver buffer, the network, the snapclient and therefore of the source-side segment River publishes, so it is ALREADY tracking this buffer and a delta on top would correct the same milliseconds twice), `applied` (no lock — the raw-position fallback absorbs none of it, so the delta is real). An UNKNOWABLE lock is treated as PRESENT: applying nothing is the safe direction. No smoothing at either transition, nor across an epoch change. **The FLOOR (`known_buffer_floor_ms`, 500 default, a SETTING and his headroom) is what the value READS as when the reading is missing — it is never a delta to subtract; zero is forbidden as a READING and every synonym for it ("uncorrected", "no correction") is the same forbidden thing.** `known_buffer_ceiling_ms` (1500) flags and does NOT clamp. **KNOWN FOLLOW-UP, not done here: on a LOCKED song a drain is corrected only as fast as the lock's own chase** (the mismatch monitor's 2 s interval × 3 confirms, at most 2 recoveries per play); bridging that chase with River's step event is filed separately | River publishes (loopback `:8097`); SPECTRA mirrors and applies it at the show clock | **Live as INGESTION, INERT as compensation** until River's bounding half goes live: every real reading today is `floor_clamped`, so the reason is `gate` and the clock is untouched | `spectra/services/known_buffer.py` (module docstring: the whole law), `spectra/services/av_sync_lead.py::show_clock_ms` (the one application point), `spectra/services/room_controls.py` (the five settings), `tests/test_known_buffer_seam.py` (the three answers, both transitions, both signs), `tests/test_known_buffer.py`, `tests/test_known_buffer_api.py` |
...
| `settings.xcorr_keep_searching_{interval_ms (5000), give_up_ms (45000)}` + `KeepSearchingConfig.{end_buffer_ms (= _XCORR_END_BUFFER_MS = 30000), max_overlap_ms (= uscore_planner._MAX_OVERLAP_MS = 1000)}` | ms | **Not signed, and NOT a member of either family** — they decide WHEN the auto-offset sweep measures and WHEN IT STOPS, never what a measurement means, and they are recorded here so nobody later reads one as a lead or an offset. When the planned windows run out without a hard lock, the play keeps placing ONE spike-targeted window (`xcorr_core.mismatch_spike`, the drift monitor's own placement) every `interval_ms` of song, admitted by the planned queue's own readiness rule (live clock past the window's end plus `_XCORR_MARGIN_MS`, compared with no offset applied) and refused if it re-measures more than `max_overlap_ms` of audio an earlier window already scored. It gives up after `give_up_ms` of song with no continued window casting a confirmation vote (`nothing_to_find`), in the song's last `end_buffer_ms` where no window is ever planned (`no_time_left`), after consecutive continued windows the envelope clip refused against the same engine offset have spanned the same `give_up_ms` of song (`nothing_admissible`), or at once for a user-verified offset. Every measurement still goes through `SweepEvaluator` unchanged, so every offset it adopts carries the sweep's existing sign — INCLUDING the envelope clip, which a continued window only reaches because it is handed the safe-shift envelope the U-Score planner assigns a window at that exact position (`uscore_planner.window_envelope` over the planner's own full-song bands and beats, `auto_offset_service.ContinuedEnvelopes`; `safe_neg_ms ≤ 0 ≤ safe_pos_ms`, relative to the engine's current offset, the same stored shape planned windows already carry — a range, not a lead or an offset), widened to ±`xcorr_save_confirm_tol_ms` (300ms, `auto_offset_service._floored_envelope`) ONLY when its total width is under the planner's own `_MIN_TOTAL_ENVELOPE_MS` (100ms) eligibility gate, and used verbatim otherwise: 124 of 563 of his uscore-v8 songs, MAYDAY among them, carry a zero-width (0, 0) envelope on every planned window, force-picked past that gate, which admits only a measurement equal to the engine's offset to the millisecond — there the floor restores a bound that never worked, 300ms staying under a one-beat twin at 120bpm (500ms); a side the planner MEASURED narrower than 300ms is a twin it found and is never widened. Planned windows keep their stored envelopes unchanged. The clip holds at EVERY ladder stage for a continued window (`auto_offset_service._envelope_exempt`): the global stage still skips it for planned and drift-recovery windows, never for a continued one — TRADE-OFF, accepted knowingly: once the engine has snapped, a continued window can no longer make a LARGE correction the envelope rejects at its position; a cold start still can (the clip skips while the engine's play-best is 0) and corrections and saves inside the envelope are untouched, so it refuses probably-wrong locks, not better ones. A CLIPPED continued window is not heard by the search ladder (counted empty it would escalate into the global stage) and its landscape is not added to the evidence accumulator (whose save and lock-and-stop never read the envelope); planned and drift-recovery windows are unchanged, and otherwise a continued window chooses and saves exactly like a planned one (the owner's ruling); `tests/test_keep_searching.py` proves an early lock, the post-lock drift monitor and the switched-off path byte-identical to the pinned pre-change sweep — every engine snap, save and history record compared | spot-effects auto-offset sweep | **Live** (2026-09-15, default on) | `services/xcorr_sweep.py` (`KeepSearching` docstring: the binding statement), `services/auto_offset_service.py::_detect_loop_xcorr`, `services/uscore_planner.py::window_envelope`, `config.py` (the two settings), `tests/test_keep_searching.py` |
| `lock_state` record's `offset_ms` / `quality` (the TopBar lock badge) | ms / unitless | **A DISPLAY COPY, not a quantity — recorded here so nobody later reads it as one.** It is `SweepEvaluator.best_offset` (and, mid-play, the same field at the moment of lock) copied verbatim into the badge's tooltip by `services/lock_state.py::note_outcome`. Its SIGN is whatever the sweep's own convention is — the badge only ever prints it, never adds it to anything, and nothing reads it back: `tests/test_lock_state_badge.py` asserts by AST that every `lock_state.*` call in the sweep is a bare statement whose value is discarded, so this can never reach an offset, a threshold or a lock decision. The module is on the reporting side of the line this document draws. | spot-effects auto-offset sweep (reported), the `/app` TopBar badge (rendered) | **Live** (2026-09-08) | `services/lock_state.py` (module docstring: the four phases and why the terminal signal is "the engine stopped searching", never window exhaustion), `web/src/components/lockBadge.ts` (the decision table), `scripts/check_lock_badge_states.mjs` |
| `lock_history.pipeline_drift` session `level_ms` (the pipeline-drift line, its alarm and its trend on both the `/app` Timing page and SPECTRA's own `/timing` page) | ms | **A DERIVED DISPLAY QUANTITY, neither LEAD nor OFFSET family.** Per session, the median over locked, quality-gated plays (`settings.xcorr_save_min_quality`) of `offset_ms − that song's fixed anchor`, where the anchor is the median of the song's gated plays inside the anchor era. Its sign is simply that subtraction: **negative = the song's winning offset has moved lower (earlier, in `offset_ms`'s own units) than at its anchor; positive = higher.** It inherits whatever `offset_ms` itself means and restates none of it. `None` means no anchor: a song with no gated play in the era, an unreadable anchors store, a store missing after it was written, or a first write that has not landed since the log evicted past what it was seeded from (every later play retries it) — never a re-derived anchor; a store that was written and then vanishes or turns unreadable is deliberately never rebuilt, since on a capped log that could only substitute a different era presented as the original. A gated in-era play whose save fails is retried on every later play while the era is open (it only ever adds that sample) and is reported as `anchor_status` `samples_pending`, or `samples_lost` once the era has closed. The era never moves on its own; only `lock_history.reanchor()` (`POST /api/lock-history/drift/reanchor`, a person's explicit, confirmed and audited act) adopts a different one. Nothing reads it back: its only consumer is `GET /api/lock-history/drift`. | spot-effects auto-offset sweep (measured, recorded by `lock_history.record`), the `/app` Timing page (`web/src/timingviz/LockHistoryPanel.tsx`) and SPECTRA's `/timing` page (`spectra/web/src/timingviz/LockHistoryPanel.tsx`), both rendering the same endpoint | **Live** (2026-09-16) | `services/lock_history.py` (`pipeline_drift` and `reanchor` docstrings; anchors store `storage/lock_history_anchors.json`, whose `reanchors` list is the audit trail), `routers/lock_history_router.py`, `tests/test_lock_history_drift.py`, `scripts/check_timing_drift.py` |
| `lock_history.pipeline_drift` session `median_residual_ms` (legacy, kept for continuity; tooltip only) | ms | **A DERIVED DISPLAY QUANTITY, neither family.** Per session, the median of `offset_ms − median(that song's gated plays 36 h–21 d older)`. Same subtraction sign as `level_ms`, but against a SLIDING baseline, so it reads the RECENT CHANGE in level, not the level: a steady ratchet reads as a small constant and a one-time step as a sign flip that decays over days. That lag is what turned a real ramp-then-step into apparent scatter twice in 2026-09 — **do not read it as a level.** No longer drives the alarm. | same as the row above (a tooltip on both Timing pages) | **Live** (display only) | `services/lock_history.py` (module comment above `_DRIFT_SESSION_GAP_S`) |
| `lock_history` drift constants: `_DRIFT_ANCHOR_ERA_DAYS` (4 d) / `_DRIFT_BASELINE_MIN_AGE_H` (36 h) / `_DRIFT_BASELINE_MAX_AGE_D` (21 d) / `_DRIFT_SESSION_GAP_S` (2 h) / `DRIFT_ALARM_MS` (1500) / `_DRIFT_STEP_MS` (2500) / `_DRIFT_RAMP_MAX_MS_PER_DAY` (1000 ms/day) / `_DRIFT_STABLE_BAND_MS` (200) | d / h / s / ms / ms per day | **Not signed, and NOT members of either family** — windows, magnitudes (compared as absolute differences) and a rate that classify and alarm on the `level_ms` row above; none is ever added to an offset. The era is the oldest 4 days of gated history the log held when the anchors store was first written (or the 4 days from the `start_at` a person chose in an explicit re-anchor), and a play is only measured against it once it is 36 h past the era's end. Session shape compares the absolute change in level since the previous qualifying session: ≤200 is `stable`; ≥2500 is `step` only when faster than 1000 ms/day over (previous session's start → this session's end), otherwise `step_or_ramp`; between the two it is `ramp` or, against the direction of the last move, `reversal`. The 1000 ms/day figure is measured, not tuned — its derivation is in the constant's own comment. | same as the rows above (classified server-side, rendered on both Timing pages) | **Live** (2026-09-16) | `services/lock_history.py` (each constant's inline comment) |
| `testbed_audio.capture_offset_ms` (a captured WAV's own sample 0, expressed in song time), applied as `frame_ms = raw_ms + capture_offset_ms_or_zero(uri)` | ms | **OFFSET family: positive = LATER, 0 = unchanged.** A librosa section boundary's own `start_ms` is placed in the CAPTURED-WAV's frame (`services/librosa_service.py` places every boundary on a beat of the WAV, not the song), but the room's fire clock and a human's authored triggers both run in SONG time — and most captures start several seconds into the song (URI-detection lag), so an unshifted cue fired that many ms EARLY. Applied at GENERATION (`spectra/services/midsong_generator.py::candidate_moments`, before `beat_snap.place_cue`'s own R3-then-R1 placement search — since 2026-09-23 BOTH the rhythmic-edge candidates and the downbeat grid are shifted by the identical offset (`_shift_song_placement`, which shifts the grid via the pre-existing `_shift_song_grid`) so the comparison stays single-frame, per `beat_snap.py`'s own "ONE FRAME, NO SHIFT" note) and at the automatic-decision intensity lookup (`spectra/services/analysis_reader.py::section_energy_at`, which a caller such as `bridge.intensity()` calls with a song-time position). `generator_key` is unaffected — still keyed on the section's own unshifted, WAV-time `start_ms`, so a later change to the capture (a recapture) or to this field updates the same stored trigger in place rather than orphaning it. **Not** `librosa_offset_ms` (the row above the master table's opening rows, `AGENTS.md`-documented as unreliable and deliberately unread) — this is a different, measured quantity from the `.npz` sidecar's own first timestamp, cached per URI (keyed on the sidecar's own `(mtime_ns, size)`, so a live recapture self-heals the cache on its next read instead of replaying a stale offset for the rest of the process life) to keep the ~200ms trigger-engine tick free of a synchronous file read | SPECTRA mid-song generation, SPECTRA automatic-decision intensity lookup | **Live** (2026-09-23) | `spectra/services/testbed_audio.py::{capture_offset_ms,capture_offset_ms_or_zero}` (the one definition), `spectra/services/midsong_generator.py::{candidate_moments,_shift_song_grid,_shift_song_placement}`, `spectra/services/analysis_reader.py::{section_energy_at,_capture_offset_for}`, `scripts/check_transition_alignment.py`, `tests/test_midsong_generator.py` |
...
### Known follow-up: the lock's chase bounds how fast a drain is corrected

This term is also what GATES the known audio buffer
(`effects_fire_later_by_ms`, the row above): while `shape_offset_ms` has a
value, SPECTRA subtracts nothing, because the lock taps `snapcast.monitor`
— MEASURED to be the monitor of the sink `snapclient` plays into, i.e.
speaker time — and is therefore already tracking that buffer.

**The limitation that leaves, stated so nobody rediscovers it: on a LOCKED
song a DRAIN is corrected only as fast as the lock's own chase.** The
mismatch monitor re-checks on a 2 s interval, needs 3 confirms, and is
capped at 2 recoveries per play (`config.xcorr_monitor_*`), so an instant
step is uncorrected for that long — where an UNLOCKED song gets River's
step event applied immediately. Bridging that chase with River's step
event (handing the lock a spike-targeted re-check, or shortening the
confirm window on a published discontinuity) is a **separate follow-up and
was deliberately not done** when the seam was ruled, 2026-09-17. It is
named here, in `spectra/services/known_buffer.py`'s docstring, and in the
`known-audio-buffer` help topic.
