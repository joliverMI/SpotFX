...
| Fish dispersal + swim burst, effect-side: `DEPART_S` (1.2 s), `LULL_LEAK_FROM`/`LULL_LEAK_TO`/`LULL_EXIT_BY` (0.4/0.72/0.92 of `LULL_GONE_AT`), `LULL_EXIT_MIN_S` (0.3 s), `TRANSITION_EXIT_BY` (0.6 of the crossfade), `BURST_ATTACK_S`/`BURST_RELEASE_S` (0.03/0.05 s) | s / phase-progress fraction / crossfade-weight fraction | Not signed. `p_dl` is an effect-clock DEADLINE a dispersing fish must be off the panel by; its speed is derived from it, never tuned. The lull fractions sit on the SpotFX-ramped `phase_progress` axis (same ~90%-then-hang caveat as Black Hole's row), converted to seconds by the progress rate read back off the effect's own phase clock. The burst has NO duration of its own: the Fish Swim Burst flare kind owns it — `hold_ms` (300) is the length and `FlareKind.trigger_offset_ms` (authored 0, OFFSET family, negative = earlier) the start, so it fires ON the trigger; a toggle computes lead 0, so nothing else shifts it | `fx/effects/fish.py` (effect-side; SPECTRA writes `phase`/`phase_progress`/`swim_burst`) | **Live** once deployed + `scripts/add_fish_swim_burst_flare.py` applied | `fx/effects/fish.py` (dispersal + swim-burst blocks near the top), `fx/VENDOR.md` #39 |
