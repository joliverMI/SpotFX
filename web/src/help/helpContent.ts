  ...
  /* ── Timing & Debug ──────────────────────────────────────────── */
  {
    ...
    entries: [
      ...
      },
      {
        ...
        keywords: 'drift alarm audio chain latency session offset moved snapclient common shift level step ramp stable',
        body: [
          ...
          'The number shown is a LEVEL: each gated play\'s winning offset minus that same song\'s own FIXED anchor (the median of its earliest gated plays), never a difference against a rolling window. The chip beside it says whether that reading is still a RAMP (moving the same direction as the last reading), a STEP (a discrete jump), or STABLE (settled within a tight band) — so a real one-time step reads as a step, not as scatter with the sign flipped. A muted "legacy" figure (a sliding 1.5–21-day-old baseline) is kept in the tooltip for continuity, but it lags a real change by design and is no longer what drives the alarm.',
