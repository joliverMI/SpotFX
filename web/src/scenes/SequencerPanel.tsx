/** Sequencing attachment panel — lives inside the Scenes-page editor for the
 * selected scene. Owns exactly the GRAPHICAL half of the interface split:
 * which likelihood curve the scene carries (named profile / inline one-off /
 * flat / not sequenced) and the curve's shape. Relationships and durations
 * (genre fit, dwell weight, affinity) render as a read-only summary — they
 * are adjusted by telling the agent, never by a form here. */
...
export default function SequencerPanel({ scene, scenes }: {
  scene: SceneV2;
  scenes: SceneV2[];
}) {
  ...
  // Draft points for the editor; keyed remount (below) resets on any
  // scene/attachment switch so drafts never leak across contexts.
  ...
  const attach = async (value: string) => {
    ...
    try {
      if (value === 'none') await attachMut.mutateAsync({ sceneId: scene.id, attachment: { kind: 'none' } });
        ...
        // Seed the one-off from whatever is currently attached.
  ...
  return (
    <div>
      <div className="card-title" style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 6 }}>
        Sequencing <HelpLink topic="sequencer-attachment" />
      </div>
