...
from fx.effects.melbank import (
    MAX_FREQ,
    MIN_FREQ,
    FrequencyRange,
)
from fx.effects.oneshots.oneshot import Oneshot
...
from fx.transitions import Transitions
from fx.utils import Teleplot, fps_to_sleep_interval, is_gap_device
...
class Virtual:
    CONFIG_SCHEMA = vol.Schema(
        {
            ...
            # SpotFX dark mode: while True, Effect._apply_config hard-clamps
            # background_color to #000000 / background_brightness to 0 on this
            # virtual, so NO write path (PUT, tween, scene, preset) can light a
            # background. Set via POST /api/virtuals partial config merge.
            vol.Optional(
                "dark_lock",
                description="Force effect backgrounds to black while set",
                default=False,
            ): bool,
            # Shape map (SpotFX): text geometry of a shaped matrix (see
            # ledfx/shapemap.py). Compiles into the gap/LED segment list plus
            # per-LED resample gather tables consumed by flush().
            vol.Optional(
                "shape_map",
                description="Shape-map text for shaped matrices ('' = none)",
                default="",
            ): str,
            vol.Optional(
                "shape_resample",
                description="Kernel-resample rendered frames onto the shape's LEDs",
                default=True,
            ): bool,
            vol.Optional(
                "shape_resample_blend",
                description="Resample kernel blend: 0 = area average, 1 = catchment max",
                default=0.35,
    ...
    def __init__(self, ledfx, config):
        ...
        # Shape map (see ledfx/shapemap.py): compiled shape + resample gather
        # tables. Both are swapped by single reference assignment so flush()
        # can take a local ref without extra locking. None = unmapped virtual,
        # flush behaves exactly as before.
        self._shape = None
        self._resample = None
        self._compile_shape()
    ...
    def activate_segments(self, segments):
        ...
        for segment in segments:
            device_id, start_pixel, end_pixel = segment[0], segment[1], segment[2]
    ...
    def validate_segment(self, segment):
        ...
        if len(segment) not in (4, 5):
            msg = f"Invalid segment format: {segment}, should be [device_id, start, end, invert] or [device_id, start, end, invert, offset]"
            valid = False

        if len(segment) == 5:
            device_id, start_pixel, end_pixel, invert, offset = segment
        else:
            device_id, start_pixel, end_pixel, invert = segment
            offset = 0
            # Preserve previously-set offset when the UI (which doesn't know
            # about offsets) submits a 4-tuple for a segment that already has
            # a non-zero offset stored. Match on (device_id, start, end).
            for existing in getattr(self, "_segments", None) or []:
                if (
                    len(existing) >= 5
                    and existing[0] == device_id
                    and existing[1] == start_pixel
                    and existing[2] == end_pixel
                ):
                    offset = existing[4]
                    break
        ...
        # Skip validation for gap devices - they are dummy placeholders
            ...
            return [device_id, start_pixel, end_pixel, invert, int(offset)]
        elif (
            start_pixel < 0
            or end_pixel < 0
            ...
            segment = [device_id, start_pixel, end_pixel, invert, int(offset)]
    ...
    def update_segments(self, segments_config):
        ...
        with self.lock:
            ...
            if _segments != self._segments:
                ...
                # Segments changed → rebuild (or guard-disable) the shape
                # resample tables against the new layout
                self._compile_resample_tables()
    ...
    def _compile_device_remap(self):
        ...
        for segment in self._segments:
            device_id = segment[0]
            device_start = segment[1]
            device_end = segment[2]
            reverse = segment[3]
            offset = segment[4] if len(segment) > 4 else 0
            ...
            # Apply per-segment rotational offset (positive shifts effect forward N LEDs)
            if offset:
                virtual_indices = np.roll(virtual_indices, offset)
    ...
    def _compile_shape(self):
        """Parse this virtual's shape_map config into a CompiledShape (or None
        when unset/invalid) and rebuild the resample tables. Never raises —
        a bad persisted map logs and falls back to unmapped behavior."""
        text = (self._config.get("shape_map") or "").strip()
        if not text:
            self._shape = None
            self._resample = None
            return
        try:
            ...
            self._shape = shapemap.parse(text)
        except Exception as e:
            _LOGGER.warning(
                "Virtual %s: shape_map failed to compile (%s) — resampling off",
                self.id, e,
            )
            self._shape = None
            self._resample = None
    ...
    def _compile_resample_tables(self):
        """Build the per-LED gather tables from the compiled shape. Guarded:
        if the virtual's current segments differ from the map-derived ones,
        resampling is disabled (never mis-scatter to wrong device indices).
        Swapped by single reference assignment — flush() takes a local ref."""
        shape = self._shape
        if shape is None:
            self._resample = None
            return
        if self._segments and [list(s) for s in self._segments] != \
                [list(s) for s in shape.segments]:
            _LOGGER.warning(
                "Virtual %s: segments differ from shape_map — resampling "
                "disabled until they are re-synced (apply the map via "
                "/api/virtuals/%s/shape)", self.id, self.id,
            )
            self._resample = None
            return
        try:
            ...
            _LOGGER.warning(
                "Virtual %s: shape gather build failed (%s)", self.id, e
            )
            self._resample = None
            return
        self._resample = {
            "device_id": shape.device_id,
            "dst": np.arange(shape.n_leds, dtype=np.int32),
            "idx": g["idx"],
            "w": g["w"],
            "live": g["live"],
            "idx1": g["idx1"],
    ...
    def _wants_lattice_exact(self) -> bool:
        """True when the active (or transitioning) effect draws pixel-exact on
        the device lattice — resampling would soften it, so flush falls back
        to identity sampling for the whole frame."""
        for e in (self._active_effect, self._transition_effect):
            if e is not None and getattr(e, "lattice_exact", False):
                return True
        return False

    def _flush_shape_resample(self, pixels, rs):
        """Shape-mapped flush: kernel-resample the rendered frame onto the
        shape's LEDs (or identity-sample for lattice-exact effects), then
        scatter to the device. Fully vectorized — ~0.1 ms for 976 LEDs vs
        1-4 ms of the per-segment Python loop it replaces."""
        device = self._ledfx.devices.get(rs["device_id"])
        if device is None or not device.is_active():
            return
        # Oneshots apply to the full frame pre-gather so their spatial window
        # semantics match the rendered image (the simple path mutates the
        # frame through slice views the same way).
        for oneshot in self._oneshots:
            oneshot.apply(pixels, 0, len(pixels))
        if self._wants_lattice_exact():
            out = pixels[rs["idx1"]]
        else:
            G = pixels[rs["idx"]]                       # (N, K, 3)
            avg = np.einsum("nk,nkc->nc", rs["w"], G)   # area-weighted mean
            b = float(self._config.get("shape_resample_blend", 0.35))
            if b <= 0.0:
                out = avg
            else:
                mx = np.where(rs["live"][:, :, None], G, 0.0).max(axis=1)
                out = mx if b >= 1.0 else (1.0 - b) * avg + b * mx
        device.update_pixels(self.id, [(out, rs["dst"])])
    ...
    def flush(self, pixels=None):
        ...
        # Choose flush path based on shape-map / complex_segments configuration

        rs = self._resample
        if (
            rs is not None
            and self._config.get("shape_resample", True)
            and self._config["mapping"] == "span"
            and not self._calibration
        ):
            self._flush_shape_resample(pixels, rs)
        elif (
    ...
    def render_calibration(self, data, device, segments, device_id):
        ...
        if self.complex_segments:
            for _, _, step, device_start, device_end, _offset in segments:
    ...
    def pixel_count(self):
        if self._config["mapping"] == "span":
            total = 0
            for segment in self._segments:
                start_pixel, end_pixel = segment[1], segment[2]
    ...
    def update_effect_config(self, effect, config_override=None):
        """
        ...
        Args:
            ...
            config_override (dict, optional): Persist this config instead of
                effect.config. Used when a param tween is in flight so the
                backing store reflects the TARGET end-state, not the
                mid-interpolation value the live effect currently holds.

        Returns:
            None
        """
        config = config_override if config_override is not None else effect.config
    ...
    def config(self, new_config):
        ...
        dark_lock_turned_on = _config.get("dark_lock", False) and not (
            self._config or {}
        ).get("dark_lock", False)
        old_shape_map = (self._config or {}).get("shape_map", "")

        setattr(self, "_config", _config)

        # Dark lock flipped on → black out the running effect's background
        # immediately (background-only update: applied in place, no effect
        # recreation). Unlocking restores nothing by itself — the client
        # re-sends its colors on the next fire.
        if dark_lock_turned_on and self._active_effect is not None:
            try:
                self._active_effect.update_config(
                    {"background_color": "#000000", "background_brightness": 0.0}
                )
            except Exception:
                _LOGGER.exception(
                    "dark_lock: failed to clamp active effect on %s", self.id
                )
        ...
        # Shape map changed → recompile shape + resample tables (reference
        # swap; safe against the flush thread). Blend/enable flags are read
        # from _config at flush time, no recompile needed.
        if _config.get("shape_map", "") != old_shape_map:
            self._compile_shape()
...
class Virtuals:
    ...
    def check_and_deactivate_devices(self):
        ...
        for virtual in self.values():
            if virtual.active:
                for segment in virtual.segments:
                    device_id = segment[0]
