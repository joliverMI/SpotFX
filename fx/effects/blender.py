...
class Blender(AudioReactiveEffect):
    NAME = "Blender"
    CATEGORY = "Matrix"
    HIDDEN_KEYS = ["background_color", "background_brightness", "background_mode", "blur"]
    ...
    def lattice_exact(self) -> bool:
        """Blender composites other virtuals' effects — delegate lattice
        exactness to its SOURCE effects so a pixel-exact effect (squiggles)
        stays identity-sampled when running behind the compositor. Any exact
        source makes the composite exact (conservative). The hidden
        per-effect config override still wins when set."""
        ...
        try:
            ...
            for key in ("foreground", "background", "mask"):
                vid = self._config.get(key)
                v = virtuals.get(vid) if vid else None
                eff = getattr(v, "active_effect", None) if v else None
                if eff is not None and eff is not self and getattr(
                    eff, "lattice_exact", False
                ):
                    return True
        except Exception:
            pass
