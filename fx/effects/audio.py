...
import numpy as np
import samplerate
from fx.compat_sounddevice import sd  # SpotFX deviation: lazy PortAudio load (was: import sounddevice as sd)
import voluptuous as vol

import fx.api.websocket
from fx.api.websocket import WEB_AUDIO_CLIENTS, WebAudioStream
from fx.config import save_config
from fx.effects import Effect
from fx.effects.math import ExpFilter
from fx.effects.melbank import FFT_SIZE, MIC_RATE, Melbanks
from fx.events import AudioDeviceChangeEvent, AudioSourceErrorEvent, Event
from fx.sendspin import SENDSPIN_AVAILABLE
from fx.sendspin.config import is_always_on as is_sendspin_always_on
    ...
    def _activate_inner(self):
        ...
        def open_audio_stream(device_idx):
            ...
            if hostapis[device["hostapi"]]["name"] == "WEB AUDIO":
                fx.api.websocket.ACTIVE_AUDIO_STREAM = (
                ...
                from fx.sendspin.stream import SendspinAudioStream
...
class AudioAnalysisSource(AudioInputSource):
    ...
    def get_freq_power(self, i, filtered=True):
        ...
        # isfinite (not just isnan): an inf leaks through np.isnan and, via
        # radial's `spin_total += impulse * spin; spin_total %= 1.0`, becomes a
        # latched NaN (inf % 1.0 == nan) that wedges the effect black.
