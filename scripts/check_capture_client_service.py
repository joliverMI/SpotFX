...
async def main():
    ...
    # ── 3b. THE --system PATH, DRIVEN FOR REAL ────────────────────────────
    #
    # WHAT THIS EXISTS FOR. Section 1b above verifies the GENERATED BYTES,
    # but it generates them ITSELF — so it modelled the installer instead of
    # running it, and the installer's own verify step was refusing every
    # kiosk install on a defect no proof here could see: it wrote the unit
    # into a bare `mktemp` file, and `systemd-analyze verify` resolves a
    # unit's TYPE from its BASENAME, so an unsuffixed name is refused
    # ("Failed to prepare filename ...: Invalid argument") before the
    # contents are read. Valid unit, refused install (River, kiosk-0,
    # systemd 257, 2026-09-07). The lesson is the same one this file's own
    # header states about `SessionCameraDouble`: a gate you MODEL is a gate
    # no proof exercises.
    #
    # NOTHING REACHES /etc. `sudo` is shimmed to a recorder that runs
    # nothing, so the `install` into /etc/systemd/system and the
    # `systemctl daemon-reload` are both swallowed — and the recorded
    # command line is itself the assertion, because it carries the exact
    # path the installer verified and would have placed.
    print("\n== 3b. the --system path, run for real (nothing reaches /etc) ==")
    if os.geteuid() == 0:
        # As root the installer needs no sudo, so there is nothing to
        # interpose and `install` would really write to /etc. Named, not
        # simulated.
        check(True, "SKIPPED as root: the sudo shim cannot be interposed, "
                    "and a real /etc write is not this proof's to make")
    else:
        sudo_log = td / "sudo-calls.log"
        sudo_dir = td / "shim-sudo"
        sudo_dir.mkdir(exist_ok=True)
        (sudo_dir / "sudo").write_text(
            "#!/bin/sh\n"
            f'printf "%s\\n" "$*" >> "{sudo_log}"\n'
            "exit 0\n")
        os.chmod(sudo_dir / "sudo", 0o755)
        r = await run_installer_async(
            "--system", "--url", url, "--device", "/dev/null",
            "--venv", str(VENV), "--no-start",
            path=f"{sudo_dir}:{BASE_ENV['PATH']}")
        check(r.returncode == 0,
              f"the installer's --system run SUCCEEDS: rc={r.returncode} "
              f"{(r.stdout[-400:] + r.stderr[-400:])!r}")
        if shutil.which("systemd-analyze"):
            check("systemd-analyze verify: the generated system unit is valid"
                  in r.stdout,
                  "and systemd's own parser accepted the unit the INSTALLER "
                  "generated, at the path the INSTALLER chose — the step "
                  "that used to refuse a valid unit over its filename")
        else:
            check("could not be verified here" in r.stdout,
                  "systemd-analyze is absent, and the installer says so "
                  "rather than claiming a verify it did not run")
        calls = sudo_log.read_text() if sudo_log.exists() else ""
        placed = [l for l in calls.splitlines() if l.startswith("install ")]
        check(len(placed) == 1,
              f"exactly one unit placement was attempted: {placed!r}")
        src = placed[0].split()[-2] if placed else ""
        dst = placed[0].split()[-1] if placed else ""
        check(os.path.basename(src) == "spectra-capture-client.service",
              f"THE VERIFIED PATH CARRIES THE UNIT'S OWN NAME — a suffix "
              f"systemd-analyze will accept, not an anonymous temp name: "
              f"{src!r}")
        check(dst == "/etc/systemd/system/spectra-capture-client.service",
              f"placed under the same basename it was verified as: {dst!r}")
        check(not os.path.exists(src) and not os.path.exists(os.path.dirname(src)),
              f"and the temp directory is gone afterwards: {src!r}")
        check("daemon-reload" in calls,
              "the ROOT manager was the one asked to reload — every "
              "systemctl call in --system scope goes through sudo")
        check(UNIT_DST.read_text() == UNIT_SRC.read_text(),
              "and the USER unit is untouched by a --system run")
