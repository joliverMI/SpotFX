#!/usr/bin/env python3
"""Read-only, real-data proof for the light-mode background-clear fix
(PR fm/spectra-light-mode-fix-import-crash, rebuilding the reverted PR #142
-- his ruling "do option three"). The FEATURE half of PR #142 was correct
and is unchanged here; what changed is that room_controls is now resolved
lazily everywhere instead of eagerly at DriftConductor/ResponseEngine
construction time (AGENTS.md's light-mode-fix-import-crash entry has the
incident writeup; tests/test_light_mode_cold_start.py is the regression
proof for that half). This script re-proves the feature only.
...
background-brightness-authority note from an earlier carry-forward
deploy): neither authors literal #000000 (both author #ff9940), so this
fix's substitution never touches them -- confirmed against real storage,
not assumed.
...
# His real room_controls.json (storage/spectra/room_controls.json, primary
# checkout): display_mode="light", display_light_bg_color="#7800be".
LIGHT_BG = "#7800be"
...
def part3_cold_start_reminder() -> None:
    print("== Part 3: reminder -- this script proves the FEATURE only ==")
    print("  A green run here says nothing about whether the service can "
          "start; that is what tests/test_light_mode_cold_start.py checks, "
          "in a genuinely fresh interpreter (subprocess), not this process.")
    print("  -> see AGENTS.md's light-mode-fix-import-crash entry.\n")
