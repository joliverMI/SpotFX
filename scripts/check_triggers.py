...
# DROP ANCHORS ITS START TO THE MARK (his ruling, 2026-08-20 — settled
# after Black Hole was tried and withdrawn as a drop-timing reference,
# data/drops-still-fire-early-star-does-not-explode/): unlike flare, a drop
# must get lead=0 UNCONDITIONALLY, never the momentary-glide END-anchor
# lead — proven here against a scene shaped EXACTLY like the momentary-
# flare proof above (same Pulse kind, same registry-smooth param, same
# qualifying band) except the response class is "drop". If this ever
# returns DICE_REROLL_GLIDE_MS instead of 0, the drop-vs-flare branch in
# _response_switch_lead_ms has regressed — his four real scenes alone
# would never have caught it, since none of their real drop bands happen
# to carry a qualifying kind today (see the STAR-shaped checks below).
drop_scene = SceneV2(
    name="Momentary Drop",
    devices=[SceneDeviceConfig(target_kind="virtual", target="v-m1",
                               effect_type="concentric")],
    ...
    responses={"drop": ResponseSpec(bands=[
        FlareBand(intensity_min=0.4, intensity_max=1.0, kinds={"Pulse": 1.0})])})
spectra_engine.responses.conductor.on_scene_fire(drop_scene, [
    {"virtual_id": "v-m1", "effect_type": "concentric",
     "config": {"gradient_scale": 1.0}, "entry_id": drop_scene.devices[0].id,
     "color_mode": "fixed"}])
lead_drop = prod_engine._response_switch_lead_ms(
    FireResponseAction(event_class="drop", intensity=1.0))
check(lead_drop == 0,
      "a drop gets zero lead even when its band carries exactly the kind "
      "shape (momentary, registry-smooth param) that would earn a flare "
      "the momentary rule's END-anchor lead — drop anchors its START to "
      "the mark instead, structurally, not just by omission")

# his own STAR scene's real drop-band shape (as of this build): a
# permanent param move (Drop patch, spin+star) plus a momentary GAIN kind
# with no params (Drop gain) — neither ever qualified for the momentary
# rule even before the explicit drop branch existed (permanent is skipped
# outright; a gain-only momentary has no params to glide). The explicit
# branch makes this true by CONSTRUCTION rather than by this coincidence
# of his current data — re-asserted here so a future edit to either kind's
# shape can't silently start borrowing the flare rule for STAR's own drop.
star_drop_scene = SceneV2(
    name="STAR-shaped Drop",
    devices=[SceneDeviceConfig(target_kind="virtual", target="v-m1",
                               effect_type="radial")],
    flare_kinds=[
        FlareKind(name="Drop patch 0.7-1", type="permanent",
                  params={"spin": ParamTarget(mode="absolute", value=0.55),
                          "star": ParamTarget(mode="absolute", value=0.0)}),
        FlareKind(name="Drop gain 0.7-1", type="momentary", gain=1.5),
    ],
    responses={"drop": ResponseSpec(bands=[
        FlareBand(intensity_min=0.7, intensity_max=1.0,
                  kinds={"Drop patch 0.7-1": 1.0, "Drop gain 0.7-1": 1.0})])})
spectra_engine.responses.conductor.on_scene_fire(star_drop_scene, [
    {"virtual_id": "v-m1", "effect_type": "radial",
     "config": {"spin": 0.25, "star": 0.0}, "entry_id": star_drop_scene.devices[0].id,
     "color_mode": "fixed"}])
lead_star_drop = prod_engine._response_switch_lead_ms(
    FireResponseAction(event_class="drop", intensity=0.84))
check(lead_star_drop == 0,
      "STAR's own real drop-band shape (permanent patch + gain-only "
      "momentary) also gets zero lead — the write lands exactly on the "
